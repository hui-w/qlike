...
var Config={version:"1.25.B8",keyboard:{changeUnit:40,timerDelay:50},Toolbar:{navUnit:20,navTimerDelay:100,zoomTimerDelay:150,marginTop:16,marginLeft:8,marginRight:8,marginBottom:16,buttonSpacing:8,buttonPadding:8,buttonSize:48},WorldMap:{maxScale:30,minScale:.5,moveTimerDelay:30,moveDuration:400,zoomTimerDelay:30,ghostCountries:[{country:"US",offset:1},{country:"CA",offset:1},{country:"CN",offset:-1},{country:"TW",offset:-1},{country:"KR",offset:-1},{country:"JP",offset:-1}]},colors:"#37C719 #E66C0F #D5312C #1B2258 #E62760 #3051EA".split(" "),
colorIndex:0};(function(a){if("replaceState"in history)a.replaceHash=function(c){"#"!==(""+c).charAt(0)&&(c="#"+c);history.replaceState("","",c)};else{var b=location.hash;a.replaceHash=function(c){location.hash!==b&&history.back();location.hash=c}}})(window);
...
a?null:"none")})},filterByYears:function(a){this.filterByYearsAndProfiles(a,["h"])},filterByYearsAndProfiles:function(a,b){if(a&&0!==a.length){b&&0!==b.length||(b=["h"]);this._clearAllElements();this._clearElementCache();var c=this._filterFlightsByYearsAndProfiles(a,b);this._renderFilteredFlights(c);this._renderFilteredRoutes(a,b);this._renderFilteredPoisWithProfiles(a,b);this.updateTransform()}else this._clearAllElements(),this._clearElementCache(),this._flightPaths=null},_filterFlightsByYears:function(a){return this._filterFlightsByYearsAndProfiles(a,
["h"])},_filterFlightsByYearsAndProfiles:function(a,b){var c=[],d=this;window.mapdata.flights.forEach(function(e){var f=[];e.forEach(function(g){var l=g.year&&-1!==a.indexOf(g.year),m=d._matchesProfile(g.profile,b);l&&m&&f.push(g)});0<f.length&&c.push(f)});return c},_renderFilteredPois:function(a){this._renderFilteredPoisWithProfiles(a,["h"])},_renderFilteredPoisWithProfiles:function(a,b){var c=this;window.mapdata.pois.forEach(function(d){var e=c._shouldShowPoiForYears(d,a),f=c._matchesProfile(d.profile,
b);e&&f&&(c.renderPoi(c.mapGroup,d,0),c._renderGhostPois(c.mapGroup,d))})},_shouldShowPoiForYears:function(a,b){if(!a.year)return!1;a=this._parseYears(a.year);for(var c=0;c<a.length;c++)if(-1!==b.indexOf(a[c]))return!0;return!1},_parseYears:function(a){return"string"===typeof a?-1<a.indexOf(",")?a.split(",").map(function(b){return parseInt(b.trim())}):[parseInt(a)]:[a]},showRoutes:function(a){this._routePaths||(this._routePaths=this.mapGroup.querySelectorAll("path.route"));var b=this._routePaths;
requestAnimationFrame(function(){for(var c=0;c<b.length;c++)b[c].updateStyle("display",a?null:"none")})},showPois:function(a){this._poiElements||(this._poiElements=this.mapGroup.querySelectorAll("circle.poi-circle, text.poi-label"));var b=this._poiElements;requestAnimationFrame(function(){for(var c=0;c<b.length;c++)b[c].updateStyle("display",a?null:"none")})},restoreOriginalRendering:function(){this._clearAllElements();Config.colorIndex=0;this.renderFlightsAndRoutes(this.mapGroup);this.renderPois(this.mapGroup);
this._applyCurrentToggleStates();this.updateTransform()},getPoi:function(a){for(var b=0;b<window.mapdata.pois.length;b++)if(window.mapdata.pois[b].key==a)return window.mapdata.pois[b];return null},getPoiPosition:function(a){a=this.getPoi(a);return null!=a?this.parseLatLon(a.latlon):{x:0,y:0}},getXY:function(a,b){return{x:2.6938*b+465.4,y:-2.6938*a+227.066}},getLatLon:function(a,b){return{lat:(b-227.066)/-2.6938,lon:(a-465.4)/2.6938}},parseLatLon:function(a){var b=(a=String(a).split(/(\d+(?:\.\d+)?)[\xb0\s]?\s*(?:(\d+(?:\.\d+)?)['\u2019\u2032\s])?\s*(?:(\d+(?:\.\d+)?)["\u201d\u2033\s])?\s*([SNEW])?/i))&&
+a[1]+(a[2]||0)/60+(a[3]||0)/3600;"S"==a[4].toUpperCase()&&(b=-b);var c=a&&+a[6]+(a[7]||0)/60+(a[8]||0)/3600;"W"==a[9].toUpperCase()&&(c=-c);return this.getXY(b,c)},getScreenCenterPointOnMap:function(){return{x:(this.frame.width/2-this.left)/this.scale,y:(this.frame.height/2-this.top)/this.scale}},_clearElements:function(a){a=this.mapGroup.querySelectorAll(a);for(var b=0;b<a.length;b++)a[b].parentNode.removeChild(a[b])},_clearAllElements:function(){this._clearElements("path.flight");this._clearElements("path.route");
this._clearElements("circle.poi-circle, text.poi-label")},_renderGhostPois:function(a,b){null===WorldMap.GHOST_POIS&&this._calculateGhostPois();WorldMap.GHOST_POIS&&-1!==WorldMap.GHOST_POIS.POSITIVE_X.indexOf(b.key)&&this.renderPoi(a,b,this.width);WorldMap.GHOST_POIS&&-1!==WorldMap.GHOST_POIS.NEGATIVE_X.indexOf(b.key)&&this.renderPoi(a,b,-this.width)},_shouldHidePoisForYearFilter:function(){return this.frame&&this.frame.toolbar&&this.frame.toolbar.selectedYears&&0===this.frame.toolbar.selectedYears.length},
_isToggleEnabled:function(a){if(!this.frame||!this.frame.toolbar)return!0;a="flight"===a?this.frame.toolbar.flightOverlay:this.frame.toolbar.routesOverlay;return!a||"none"!==a.attr("stroke")},_updatePoiCircles:function(a){for(var b=this.mapGroup.querySelectorAll("circle.poi-circle"),c=this.mapGroup.querySelectorAll("text.poi-label"),d={},e=0;e<c.length;e++){var f=c[e],g=f.attr("data-poi-key");f=f.attr("data-level");f=Util.isNull(f)?0:parseFloat(f);d[g]=!a&&this.scale>=f}for(a=0;a<b.length;a++)c=b[a],
g=c.attr("data-poi-key"),d[g]?(g=1/this.scale*4,1>this.scale&&(g/=2),c.attr("r",g),c.attr("fill","#E66C0F")):(g=1/this.scale*1.5,1>this.scale&&(g/=2),c.attr("r",g),c.attr("fill","#999999"))},_updatePoiLabels:function(a){for(var b=this.mapGroup.querySelectorAll("text.poi-label"),c=0;c<b.length;c++){var d=b[c],e=d.attr("data-level");Util.isNull(e)&&(e=0);a?"none"!==d.getStyle("display")&&d.updateStyle("display","none"):this.scale>=e?("none"===d.getStyle("display")&&d.updateStyle("display",""),d.attr("font-size",
1/this.scale*12)):"none"!==d.getStyle("display")&&d.updateStyle("display","none")}},_renderFilteredFlights:function(a){if(this._isToggleEnabled("flight")){var b=this;Config.colorIndex=0;a.forEach(function(c){c.forEach(function(d){b.renderFlight(b.mapGroup,d.from,d.to,d.crossPacific,d.direction)});Config.colorIndex++})}},_renderFilteredRoutes:function(a,b){b&&0!==b.length||(b=["h"]);if(this._isToggleEnabled("routes")){var c=this;window.mapdata.routes.forEach(function(d){var e=d.year&&-1!==a.indexOf(d.year),
f=c._matchesProfile(d.profile,b);e&&f&&c.renderRouteGroup(c.mapGroup,d.path)})}},_matchesProfile:function(a,b){a&&""!==a.trim()||(a="h");a=a.split(",").map(function(e){return e.trim()}).filter(function(e){return 0<e.length});0===a.length&&(a=["h"]);for(var c=0;c<b.length;c++){var d=b[c].trim();if(0!==d.length&&-1===a.indexOf(d))return!1}return!0},_applyCurrentToggleStates:function(){this.frame&&this.frame.toolbar&&(this._isToggleEnabled("flight")||this.showFlights(!1),this._isToggleEnabled("routes")||
this.showRoutes(!1))}};function Toolbar(a){this.frame=a;this.zoomRatio=1;this.naviSpeed={x:0,y:0};this.buttonCount=0;this.yearFilter=this.flightOverlay=this.routesOverlay=this.onNaviRestore=this.onNavigate=this.onZoomRestore=this.onZoom=this.onZoomOut=this.onZoomIn=this.naviGroup=this.toolSub=this.toolGroup=this.scaleGroup=this.naviTimer=this.zoomTimer=null;this.selectedYears=[];this.selectedProfiles=["h"];this.filterCollapsed=!0}
...
["h"]},applyProfileFilter:function(){1===this.selectedProfiles.length&&"h"===this.selectedProfiles[0]?Util.setQueryString("profile",""):Util.setQueryString("profile",this.selectedProfiles.join(","))},initYearFilter:function(){var a=this.getUniqueFlightYears();this.selectedYears=a.slice();var b=Util.getUrlHash("years"),c=!1;if(b)if(c=!0,"none"===b)this.selectedYears=[];else try{this.selectedYears=b.split(",").map(function(d){d=parseInt(d.trim(),10);return 1900<=d&&2100>=d?d:null}).filter(function(d){return null!==
d&&-1!==a.indexOf(d)})}catch(d){this.selectedYears=a.slice()}this.renderYearFilter();c||this.selectedYears.length!==a.length?this.applyYearFilter():this.frame.map.filterByYearsAndProfiles(this.selectedYears,this.selectedProfiles||["h"])},renderYearFilter:function(){var a=this.getUniqueFlightYears();0===a.length||this.yearFilter||(this.yearFilter=this.frame.map.getExtraGroup().createChildNS("g",{id:"year-filter-group",class:"year-filter-svg"}),this._createYearFilterBackground(a),this._createYearCheckboxes(a),
this.positionYearFilter())},_createYearFilterBackground:function(a){a=this._getFilterDimensions(a);this.yearFilter.createChildNS("rect",{x:2,y:2,width:a.width,height:a.height,fill:"rgba(0, 0, 0, 0.1)",rx:6,ry:6});this.yearFilter.createChildNS("rect",{x:0,y:0,width:a.width,height:a.height,fill:"rgba(255, 255, 255, 0.97)",stroke:"#bbb","stroke-width":1,rx:6,ry:6})},_createYearCheckboxes:function(a){var b=this,c=8;this._createFilterTitleBar(8,c);c+=24;c+=8;c+=10;this.filterCollapsed||(this.createSVGCheckbox(this.yearFilter,
13,c,"All Years",this.selectedYears.length===a.length,function(){b._toggleAllYears(a)}),c+=24,a.forEach(function(d){b.createSVGCheckbox(b.yearFilter,13,c,d.toString(),-1!==b.selectedYears.indexOf(d),function(){b._toggleYear(d)});c+=22}),this._addVersionNumber(a))},_getFilterDimensions:function(a){var b=Math.max.apply(Math,a.concat(["All Years","Years"]).map(function(c){return 9*c.toString().length}));return{width:Math.max(140,32+b+16+8),height:this.filterCollapsed?40:22*a.length+98}},_createFilterTitleBar:function(a,
b){var c=this,d=this.getUniqueFlightYears();d=Math.max.apply(Math,d.concat(["All Years","Years"]).map(function(f){return 9*f.toString().length}));var e=Math.max(140,32+d+16+8);d=this.yearFilter.createChildNS("rect",{x:0,y:b,width:e,height:24,fill:"rgba(240, 240, 240, 0.3)",stroke:"none",rx:4,ry:4,style:"cursor: pointer;"});this.yearFilter.createChildNS("text",{x:a+4,y:b+15,fill:"#333","font-size":"12px","font-family":"Arial, sans-serif","font-weight":"bold"},"Years");a=e-26;e=b+0;b=this.yearFilter.createChildNS("rect",
{x:a-6,y:e,width:24,height:24,fill:"transparent",stroke:"none",style:"cursor: pointer;"});a=this.yearFilter.createChildNS("path",{d:this.filterCollapsed?"M3,2 L9,6 L3,10 Z":"M2,3 L6,9 L10,3 Z",fill:"#666",stroke:"none",transform:"translate("+a+", "+(e+6)+")",style:"cursor: pointer;"});e=function(f){f.addEventListener("click",function(g){g.preventDefault();g.stopPropagation();c._toggleFilterCollapse()});f.addEventListener("touchstart",function(g){g.preventDefault();g.stopPropagation();g._handled=!0});
f.addEventListener("touchend",function(g){g.preventDefault();g.stopPropagation();g._handled=!0;c._toggleFilterCollapse()})};e(d);e(b);e(a)},_addVersionNumber:function(a){if(!this.filterCollapsed){a=22*a.length+86;var b=this.yearFilter.querySelector("rect:nth-child(2)");b=b?parseInt(b.getAttribute("width"))/2:70;this.yearFilter.createChildNS("text",{x:b,y:a,fill:"#999","font-size":"9px","font-family":"Arial, sans-serif","text-anchor":"middle","font-style":"italic"},"v"+Config.version)}},_toggleFilterCollapse:function(){this.filterCollapsed=
!this.filterCollapsed;this._refreshYearFilter()},_refreshYearFilter:function(){var a=null;this.yearFilter&&(a=this.yearFilter.getAttribute("transform"),this.yearFilter.parentNode.removeChild(this.yearFilter),this.yearFilter=null);this.renderYearFilter();a&&this.yearFilter&&this.yearFilter.setAttribute("transform",a)},_toggleAllYears:function(a){this.selectedYears=this.selectedYears.length===a.length?[]:a.slice();this.updateSVGCheckboxes();this.applyYearFilter()},_toggleYear:function(a){var b=this.selectedYears.indexOf(a);
-1!==b?this.selectedYears.splice(b,1):this.selectedYears.push(a);this.selectedYears.sort();this.updateSVGCheckboxes();this.applyYearFilter()},positionYearFilter:function(a,b){this.yearFilter&&(a=a||(this.frame&&this.frame.map?this.frame.map.width:1E3),b=this.yearFilter.querySelectorAll("rect"),b=1<b.length?parseInt(b[1].getAttribute("width")):130,this.yearFilter.attr("transform","translate("+(a-b-15)+", 15)"))},createSVGCheckbox:function(a,b,c,d,e,f){a=a.createChildNS("g",{class:"svg-checkbox","data-label":d});
b=this._createCheckboxElements(a,b,c,16,d,e);this._addCheckboxEventHandlers(b.bg,b.text,b.touchArea,f);a.checkmarkElement=b.checkmark;a.isChecked=e;return a},_createCheckboxElements:function(a,b,c,d,e,f){var g=a.createChildNS("rect",{x:b-8,y:c-d-4,width:d+8+(8*e.toString().length+10)+20,height:28,fill:"transparent",stroke:"none",style:"cursor: pointer"}),l=a.createChildNS("rect",{x:b,y:c-d+2,width:d,height:d,fill:"white",stroke:"#666","stroke-width":1,rx:3,ry:3,style:"cursor: pointer"});f=a.createChildNS("path",
{d:"M"+(b+3)+","+(c-6)+" L"+(b+7)+","+(c-2)+" L"+(b+13)+","+(c-10),stroke:"#2c5aa0","stroke-width":2.5,fill:"none","stroke-linecap":"round","stroke-linejoin":"round",style:f?"display: block":"display: none"});a=a.createChildNS("text",{x:b+d+8,y:c,fill:"#333","font-size":"13px","font-family":"Arial, sans-serif",style:"cursor: pointer"},e);return{bg:l,checkmark:f,text:a,touchArea:g}},_addCheckboxEventHandlers:function(a,b,c,d){var e=0,f=a.attr("fill"),g=null,l=!1,m=function(h){h.preventDefault();h.stopPropagation();
h._handled=!0;g={x:h.clientX,y:h.clientY};l=!1;a.attr("fill","#e6f3ff")},p=function(h){g&&5<Math.abs(h.clientX-g.x)+Math.abs(h.clientY-g.y)&&(l=!0,a.attr("fill",f))},q=function(h){h.preventDefault();h.stopPropagation();h._handled=!0;g&&!l&&d();g=null;l=!1;a.attr("fill",f)},r=function(h){!g&&300<Date.now()-e&&a.attr("fill","#f0f0f0")},n=function(h){g?(g=null,l=!1,a.attr("fill",f)):300<Date.now()-e&&a.attr("fill",f)},k=function(h){h.preventDefault();h.stopPropagation();h._handled=!0;e=Date.now();a.attr("fill",
"#e6f3ff")},t=function(h){h.preventDefault();h.stopPropagation();h._handled=!0;e=Date.now();a.attr("fill",f);d()},u=function(h){h.preventDefault();h.stopPropagation();h._handled=!0;e=Date.now();a.attr("fill",f)},v=function(h){h.preventDefault();h.stopPropagation();h._handled=!0};[a,b,c].forEach(function(h){h.addEventListener("mousedown",m);h.addEventListener("mousemove",p);h.addEventListener("mouseup",q);h.addEventListener("mouseenter",r);h.addEventListener("mouseleave",n);h.addEventListener("touchstart",
k,{passive:!1});h.addEventListener("touchend",t,{passive:!1});h.addEventListener("touchcancel",u,{passive:!1});h.addEventListener("touchmove",v,{passive:!1})})},updateSVGCheckboxes:function(){if(this.yearFilter)for(var a=this.getUniqueFlightYears(),b=this.yearFilter.querySelectorAll("g.svg-checkbox"),c=0;c<b.length;c++){var d=b[c],e=d.getAttribute("data-label");e="All Years"===e?this.selectedYears.length===a.length:-1!==this.selectedYears.indexOf(parseInt(e));d.isChecked=e;d.checkmarkElement&&(d.checkmarkElement.style.display=
e?"block":"none")}},updateYearCheckboxes:function(){this.updateSVGCheckboxes()},updateAllCheckbox:function(){this.updateSVGCheckboxes()},applyYearFilter:function(){this.frame.map.filterByYears(this.selectedYears);var a=this.getUniqueFlightYears();a=0===this.selectedYears.length?"none":this.selectedYears.length===a.length?"":this.selectedYears.join(",");Util.setUrlHash("years",a);this.applyProfileFilter()},adjustPosition:function(a,b){this.scaleGroup.attr("transform",Util.format_str("translate({left}, {top})",
{left:a-Config.Toolbar.marginRight-Config.Toolbar.buttonSize,top:b-Config.Toolbar.marginBottom-(2*Config.Toolbar.buttonSize+Config.Toolbar.buttonSpacing)}));this.toolGroup.attr("transform",Util.format_str("translate({left}, {top})",{left:Config.Toolbar.marginLeft,top:Config.Toolbar.marginTop}));this.naviGroup.attr("transform",Util.format_str("translate({left}, {top})",{left:Config.Toolbar.marginLeft,top:b-Config.Toolbar.marginBottom-(2*Config.Toolbar.buttonSize+Config.Toolbar.buttonSpacing)}));this.positionYearFilter(a,
b)},renderButton:function(a,b,c,d,e,f,g,l,m){var p=this,q=0;a=a.createChildNS("g",{transform:"translate({0}, {1})".format((Config.Toolbar.buttonSize+Config.Toolbar.buttonSpacing)*c,(Config.Toolbar.buttonSize+Config.Toolbar.buttonSpacing)*b)});var r=a.createChildNS("path",{fill:f?"RGBA(0,0,0,1)":"none",stroke:e?"RGBA(0,0,0,1)":"none","stroke-width":4,"stroke-linejoin":"round",d:d,transform:"translate({0}, {0})".format(Config.Toolbar.buttonPadding)}),n=a.createChildNS("rect",{fill:"RGBA(255,255,255,0.4)",
stroke:g?"#333":"none","stroke-width":1,x:0,y:0,rx:Config.Toolbar.buttonSize/8,ry:Config.Toolbar.buttonSize/8,width:Config.Toolbar.buttonSize,height:Config.Toolbar.buttonSize});n.addEventListener("touchstart",function(k){k.preventDefault();k.stopPropagation();k._handled=!0;q=Date.now();p.highlightIcon(this.parentNode.childNodes[0],!0);"function"===typeof l&&l(k,r,n)},!0);n.addEventListener("touchend",function(k){k.preventDefault();k.stopPropagation();k._handled=!0;q=Date.now();p.highlightIcon(this.parentNode.childNodes[0],
!1);"function"===typeof m&&m(k,r,n)},!0);n.addEventListener("touchcancel",function(k){k.preventDefault();k.stopPropagation();k._handled=!0;q=Date.now();p.highlightIcon(this.parentNode.childNodes[0],!1);"function"===typeof m&&m(k,r,n)},!0);n.addEventListener("touchmove",function(k){k.preventDefault();k.stopPropagation();k._handled=!0},!0);n.addEventListener("mousedown",function(k){500>Date.now()-q||(p.highlightIcon(this.parentNode.childNodes[0],!0),"function"===typeof l&&l(k,r,n),k.stopPropagation())},
!0);n.addEventListener("mouseup",function(k){500>Date.now()-q||(p.highlightIcon(this.parentNode.childNodes[0],!1),"function"===typeof m&&m(k,r,n),k.stopPropagation())},!0);n.addEventListener("mouseout",function(k){500>Date.now()-q||(p.highlightIcon(this.parentNode.childNodes[0],!1),"function"===typeof m&&m(k,r,n),k.stopPropagation())},!0);n.addEventListener("mousemove",function(k){k.stopPropagation()},!0);return{icon:r,overlay:n}},startZoomTimer:function(){var a=this;this.zoomTimer&&(clearTimeout(this.zoomTimer),
this.zoomTimer=null);var b=function(){1!==a.zoomRatio&&"function"===typeof a.onZoom?(requestAnimationFrame(function(){a.onZoom(a.zoomRatio)}),a.zoomTimer=setTimeout(b,Config.Toolbar.zoomTimerDelay)):a.zoomTimer=null};b()},startNaviTimer:function(){var a=this;this.naviTimer&&(clearTimeout(this.naviTimer),this.naviTimer=null);var b=function(){0===a.naviSpeed.x&&0===a.naviSpeed.y||"function"!==typeof a.onNavigate?a.naviTimer=null:(requestAnimationFrame(function(){a.onNavigate(a.naviSpeed.x,a.naviSpeed.y)}),
a.naviTimer=setTimeout(b,Config.Toolbar.navTimerDelay))};b()},highlightIcon:function(a,b){"none"!=a.attr("stroke")&&a.attr("stroke",b?"#ff0000":"#000000");"none"!=a.attr("fill")&&a.attr("fill",b?"#ff0000":"#000000")}};
