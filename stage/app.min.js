...
var ToolTypes={None:0,Picker:1,Move:2,Crop:4,Line:8,Rectangle:16,Ellipse:32,Polygon:64,Pencil:128,Pen:256,Label:512,Map:1024,Poi:2048,Block:4096,Eraser:8192};var SerializeType={TypeOnly:1,Abstract:2,JSON:4,SVG:8,Canvas:16};
function log(args){var $jscomp$restParams=[];for(var $jscomp$restIndex=0;$jscomp$restIndex<arguments.length;++$jscomp$restIndex)$jscomp$restParams[$jscomp$restIndex-0]=arguments[$jscomp$restIndex];{var args$0=$jscomp$restParams;var now=(new Date).toISOString();console.log.apply(console,["["+now+"]"].concat($jscomp.arrayFromIterable(args$0)))}}
...
return this.update(true)}return false},getAllProjectIds:function(){if(!this.update(false))return[];if(!this.payloadObject.hasOwnProperty("projects"))return[];var projectIds=[];for(var key in this.payloadObject.projects)if(this.payloadObject.projects.hasOwnProperty(key))projectIds.push(key);return projectIds},isProjectNameExists:function(name){if(!this.update(false))return false;if(!this.payloadObject.hasOwnProperty("projects"))return false;for(var key in this.payloadObject.projects)if(this.payloadObject.projects.hasOwnProperty(key)){var project=
this.payloadObject.projects[key];if(project&&project.name===name)return true}return false},clear:function(){return this.clearCache(Config.LocalStorage.mainKey)},update:function(serialize){if(serialize){if(!this.payloadObject.hasOwnProperty("_version"))this.payloadObject._version=this.currentVersion;var saved=this.saveToCache(Config.LocalStorage.mainKey,JSON.stringify(this.payloadObject));return saved}else try{var json=this.loadFromCache(Config.LocalStorage.mainKey);if(json!=null){this.payloadObject=
JSON.parse(json);this.checkAndUpgradeVersion();return true}else return false}catch(err){log("Error:",err);return false}},saveToCache:function(key,str){if(!localStorage)return false;try{localStorage.setItem(key,str);return true}catch(e){log("Error saving to localStorage:",e);return false}},loadFromCache:function(key){if(!localStorage)return null;try{var str=localStorage.getItem(key);return str}catch(e){log("Error loading from localStorage:",e);return null}},clearCache:function(key){if(!localStorage)return false;
try{localStorage.removeItem(key);return true}catch(e){log("Error clearing localStorage:",e);return false}}});window.QCache=QCache})();function AppMenu(pane){this.pane=pane;this.width=200;this.title="Application"}
...
var field=settings.createChild("div",{class:"field no-select"});var input=field.createChild("input",{id:"project-name-input",type:"text",class:"text-input",style:"width: 170px; margin-left: 10px;",value:this.getWorkspace().painting.name});input.addEventListener("change",function(){that.getWorkspace().painting.name=this.value;that.getWorkspace().autoSave();that.getWorkspace().drawingBoard.showMessage("Project name updated to "+this.value)});this.nameInput=input;button.addEventListener("click",function(){that.pane.selectMenuItem(this.parentNode)},
false);return menuItem},setProjectName:function(text){this.nameInput.value=text},getWorkspace:function(){return this.pane.workspace},getDrawingBoard:function(){return this.pane.workspace.drawingBoard},renderMenuItem:function(wrapper,label,configKey,callback){var button=wrapper.createChild("div",{class:"text-item no-select"},label);button.addEventListener("click",function(e){if(this.hasClassName("selected")){this.removeClassName("selected");eval(configKey+" = false")}else{this.addClassName("selected");
eval(configKey+" = true")}if(typeof callback=="function")callback()},false);if(eval(configKey))button.addClassName("selected")},renderCheckBox:function(wrapper,label,configKey,callback){var button=wrapper.createChild("div",{class:"checkbox-item no-select"},label);var icon=button.createChildNS("svg",{class:"icon-checkbox",width:24,height:24});var gChecked=icon.createChildNS("g",{class:"icon-group-checked"});gChecked.createChildNS("path",{fill:"#ffffff",d:"M6.534,9.467l-1.867,1.867l6,6L24,4l-1.865-1.867L10.667,13.6L6.534,9.467z M21.334,21.333H2.667V2.667H16V0H2.667C1.201,0,0,1.2,0,2.667v18.667C0,22.8,1.201,24,2.667,24h18.667C22.801,24,24,22.8,24,21.333V10.667h-2.666V21.333z"});
...
boundary:frame.getBoundary()}},setCanvas:function(jsonObject){var frame=this.workspace.drawingBoard.canvasFrame.transFrame;var position=jsonObject.position;var boundary=jsonObject.boundary;frame.moveTo(position.x,position.y,false);frame.transformTo(boundary.left,boundary.top,boundary.right,boundary.bottom,true)},serialize:function(type){var o={name:this.name,canvas:this.getCanvas(),shapes:[]};for(var i=0;i<this.shapes.length;i++)o.shapes.push(this.shapes[i].toJsonObject());if(type=="JSON")return o;
else if(type=="STRING")return JSON.stringify(o);else return null},deserialize:function(obj,type){this.workspace.autoSaveEnabled=false;this.removeAllShapes();var o=null;if(type=="JSON")o=obj;else if(type=="STRING")o=JSON.stringify(o);else{this.workspace.autoSaveEnabled=true;return false}this.setCanvas(o.canvas);this.name=o.name||"";for(var i=0;i<o.shapes.length;i++){var shape=BaseShape.fromJsonObject(o.shapes[i]);this.appendShape(shape)}this.workspace.autoSaveEnabled=true;return true}};
...
sub.createChild("div",{class:"sub-content"});switch(subId){case 0:var line=subContent.createChild("div",{class:"sub-item"});line.createChild("div",{class:"link"},"Create a new project from scratch").addEventListener("click",function(){that.openProject()},false);break;case 1:var line=subContent.createChild("div",{class:"sub-item"});var form=line.createChild("form",{action:QStage.getAppUrl()});var txtId=form.createChild("input",{type:"text",class:"text",placeholder:"Project ID",name:"id"});var btnGo=
form.createChild("input",{type:"button",class:"button",value:"Retrieve"});btnGo.addEventListener("click",function(){if(QStage.isValidId(txtId.value))that.openProject(txtId.value)},false);break;case 2:if(Config.General.offlineMode===true){var line=subContent.createChild("div",{class:"sub-item"});var fileInput=line.createChild("input",{type:"file",class:"file",id:"fileToUpload",accept:".json"});var handleFileUpload=function(){var file=fileInput.files[0];if(!file)return;var reader=new FileReader;reader.onload=
function(e){try{var fileContent=e.target.result;var jsonObj=null;var projectId=null;try{jsonObj=JSON.parse(fileContent);var filename=file.name;var match=filename.match(/project_([^.]+)\.json/);if(match&&QStage.isValidId(match[1]))projectId=match[1];else projectId=QStage.generateNewProjectId()}catch(jsonError){fileContent=fileContent.replace(/^\xEF\xBB\xBF/,"");if(fileContent.length>=32){projectId=fileContent.substring(0,32);var base64Payload=fileContent.substring(32);try{var decodedPayload=atob(base64Payload);
jsonObj=JSON.parse(decodedPayload)}catch(decodeError){throw new Error("Failed to decode file. Invalid format.");}}else throw new Error("File format is invalid. File is too short.");}if(jsonObj&&projectId){that.workspace.reset();QStage.setUrlHash("id",projectId);QCache.setProject(projectId,jsonObj);that.workspace.painting.deserialize(jsonObj,"JSON");that.setDisplay(false);fileInput.value=""}else throw new Error("Failed to parse project data.");}catch(error){alert("Error uploading file: "+error.message);
fileInput.value=""}};reader.onerror=function(){alert("Error reading file.");fileInput.value=""};reader.readAsText(file)};fileInput.addEventListener("change",handleFileUpload,false)}else{var line=subContent.createChild("form",{class:"sub-item",action:"http://stage.qlike.com/repository/upload.php",method:"post",enctype:"multipart/form-data"});line.createChild("input",{type:"file",class:"file",name:"fileToUpload",id:"fileToUpload",placeholder:"Project ID"});line.createChild("input",{type:"submit",class:"button",
...
0)subContent.removeChild(subContent.firstChild);for(var i=0;i<projectIds.length;i++){var projectId=projectIds[i];var project=QCache.getProject(projectId);var line=subContent.createChild("div",{class:"sub-item",style:"display: flex; align-items: center; justify-content: space-between;"});var linkDiv=line.createChild("div",{class:"link",title:projectId,"data-id":projectId,style:"flex: 1;"});var namedisplay=project.name?" ("+project.name+")":"";linkDiv.textContent=projectId+namedisplay;linkDiv.addEventListener("click",
function(){that.openProject(this.getAttribute("data-id"))},false);var deleteBtn=line.createChild("button",{class:"button",style:"margin-left: 10px; padding: 4px 8px; font-size: 12px;","data-id":projectId});deleteBtn.textContent="Delete";deleteBtn.addEventListener("click",function(e){e.stopPropagation();var idToDelete=this.getAttribute("data-id");if(confirm("Are you sure you want to delete project: "+idToDelete+"?"))if(QCache.deleteProject(idToDelete))that.refreshProjectList();else alert("Failed to delete project.")},
false)}if(projectIds.length<=0)subContent.createChild("div",{class:"sub-item"},"No record")}catch(err){log(err);while(subContent.childNodes.length>0)subContent.removeChild(subContent.firstChild);subContent.createChild("div",{class:"sub-item"},"Error loading project list")}else{var url="http://stage.qlike.com/repository/?action=pull";QStage.sendPostRequest(url,null,function(responseText){try{while(subContent.childNodes.length>0)subContent.removeChild(subContent.firstChild);var obj=JSON.parse(responseText);
for(var i=0;i<obj.payload.length;i++){var line=subContent.createChild("div",{class:"sub-item"});line.createChild("div",{class:"link",title:obj.payload[i].id,"data-id":obj.payload[i].id},obj.payload[i].id).addEventListener("click",function(){that.openProject(this.getAttribute("data-id"))},false)}if(obj.payload.length<=0)subContent.createChild("div",{class:"sub-item"},"No record")}catch(err$1){log(err$1)}},function(errorCode){log(errorCode)})}},renderAppIcon:function(wrapper){var icon=wrapper.createChildNS("svg",
{width:32,height:32});var g=icon.createChildNS("g");var size=32;var padding=size*10/300;var path="M{0},{1} H{2} L {3},{4} H {0} Z".format(size/3+padding,padding,size*2/3,size-padding,size/3-padding);g.createChildNS("path",{fill:"#22CB20",stroke:"#22CB20","stroke-width":padding,"stroke-linejoin":"round",d:path});g.createChildNS("path",{fill:"#06ACD4",stroke:"#06ACD4","stroke-width":padding,"stroke-linejoin":"round",d:path,transform:"translate(0,0), rotate({1},{0},{0})".format(size/2,1*90)});g.createChildNS("path",
{fill:"#FFCC00",stroke:"#FFCC00","stroke-width":padding,"stroke-linejoin":"round",d:path,transform:"translate(0,0), rotate({1},{0},{0})".format(size/2,2*90)});g.createChildNS("path",{fill:"#FA5151",stroke:"#FA5151","stroke-width":padding,"stroke-linejoin":"round",d:path,transform:"translate(0,0), rotate({1},{0},{0})".format(size/2,3*90)});return icon}};
...
Workspace.prototype={init:function(){this.painting=new Painting(this);this.eventManager=new EventManager(this);this.drawingBoard=new DrawingBoard(this);this.scaleplate=new Scaleplate(this,0,0);this.slidePane=new SlidePane(this);this.objectManager=new ObjectManager(this);this.textPanel=new TextPanel(this);this.promptDialog=new PromptDialog(this)},newProject:function(){var defaultName="Untitled";var i=1;var newName=defaultName;while(QCache.isProjectNameExists(newName)){newName=defaultName+" "+i;i++}this.painting.name=
newName;this.slidePane.settingMenu.setProjectName(newName);var newId=QStage.generateNewProjectId();QStage.setUrlHash("id",newId);this.reset(true);this.autoSave()},reset:function(enableAutoSave){this.drawingBoard.reset();this.slidePane.reset();this.painting.reset();if(enableAutoSave)this.autoSaveEnabled=true},showStartPage:function(show){if(show)if(this.startPage==null){this.startPage=new StartPage(this);this.startPage.renderInto(this.startWrapper,this.width,this.height);this.stageWrapper.updateStyle("left",
"{0}px".format(this.width))}else this.startPage.setDisplay(true);else if(this.startPage!=null)this.startPage.setDisplay(false)},render:function(){var that=this;this.width=document.documentElement.clientWidth-2*Config.Workspace.margin;this.height=document.documentElement.clientHeight-2*Config.Workspace.margin;var rootElement=document.body;this.stageWrapper=rootElement.createChild("div",{id:"workspace"});this.startWrapper=rootElement.createChild("div",{id:"start-page"});this.scrimWrapper=rootElement.createChild("div",
{class:"scrim",style:"display: none"});var css3support=QStage.supportsTransitions();this.stageWrapper.style[css3support.prefixedProperty]="left 0.5s ease-in";this.stageWrapper.addEventListener(css3support.prefixedEventName,function(){});this.slidePane.renderIntoWorkspace();this.scaleplate.renderIntoWorkspace();this.drawingBoard.renderIntoWorkspace();this.objectManager.renderIntoWorkspace();this.textPanel.renderIntoWorkspace();this.promptDialog.renderIntoWorkspace();this.sizeChanged();this.painting.onShapeChanged=
function(newShape){if(newShape!=null&&newShape.committed){that.slidePane.showPropertyMenu(newShape);that.painting.setFill(newShape.prop("fill").color,newShape.prop("fill").opacity);that.painting.setStroke(newShape.prop("stroke").color,newShape.prop("stroke").opacity,newShape.prop("stroke").width)}else that.slidePane.showPropertyMenu(null);if(newShape!=null)that.objectManager.selectShapes([newShape]);else that.objectManager.selectShapes([])};this.painting.onFillChanged=function(color,opacity){that.slidePane.setFill(color,
opacity)};this.painting.onStrokeChanged=function(color,opacity,width){that.slidePane.setStroke(color,opacity,width)};this.painting.onShapeAppended=function(appendedShape){that.objectManager.appendShape(appendedShape)};this.painting.onShapeRemoved=function(removedShape,removedIndex){that.objectManager.removeShape(removedShape,removedIndex)};this.painting.onShapeSorted=function(shape,oldIndex,newIndex){that.objectManager.sortShape(shape,oldIndex,newIndex)};this.painting.onToolChanged=function(oldToolType,
newToolType){that.slidePane.selectToolType(newToolType);if(newToolType==ToolTypes.Pen&&that.painting.selectedShape!=null&&that.painting.selectedShape.isPathShape()){that.painting.selectedShape.switchPathEditing(true);that.slidePane.showPropertyMenu(null)}else this.selectShape(null);if(oldToolType==ToolTypes.Crop)that.drawingBoard.canvasFrame.setEnabled(false);if(newToolType==ToolTypes.Crop)that.drawingBoard.canvasFrame.setEnabled(true)};this.painting.onProjectChanged=function(){that.autoSave()};this.drawingBoard.onBoardMoved=
function(offsetX,offsetY){that.scaleplate.changeOffset(offsetX,offsetY)};this.drawingBoard.onCaptured=function(){that.slidePane.selectMenuItem(null)};this.slidePane.fillMenu.onColorChanged=function(color,opacity,width){var shape=that.painting.selectedShape;if(shape!=null)shape.prop("fill",{color:color,opacity:opacity});that.painting.setFill(color,opacity)};this.slidePane.strokeMenu.onColorChanged=function(color,opacity,width){var shape=that.painting.selectedShape;if(shape!=null)shape.prop("stroke",
{color:color,opacity:opacity,width:width});that.painting.setStroke(color,opacity,width)};function checkWindowsize(){var nWidth=document.documentElement.clientWidth-2*Config.Workspace.margin;var nHeight=document.documentElement.clientHeight-2*Config.Workspace.margin;if(nWidth!=that.width||nHeight!=that.height){that.width=nWidth;that.height=nHeight;that.sizeChanged()}}setInterval(checkWindowsize,200);var animationNum=QStage.getQueryStringByName("animation");if(animationNum!=null&&animationNum>0){var addIcon=
function(){stage.addIcon();if(stage.icons.length<animationNum)setTimeout(addIcon,1E3)};if(animationNum>10)animationNum=10;var containerGroup=this.drawingBoard.underGroup;var svgWidth=this.drawingBoard.svgSize.width;var svgHeight=this.drawingBoard.svgSize.height;var stage=new Stage(containerGroup,svgWidth,svgHeight);addIcon();this.slidePane.wrapper.updateStyle("display","none")}this.loadWithUrlId()},saveToCache:function(){var projectId=QStage.getUrlHash("id");if(!projectId||projectId.length===0)return false;
var jsonObj=this.painting.serialize("JSON");var saved=QCache.setProject(projectId,jsonObj);return saved},getWrapper:function(){return this.stageWrapper},isSmallScreen:function(){return this.width<=480||this.height<=480},isLandscape:function(){return this.width>this.height},switchDeveloperMode:function(devMode){if(Config.General.developerMode==devMode)return;Config.General.developerMode=devMode;var displayStyle=devMode?null:"none";QStage.$("hidden_map_tools").updateStyle("display",displayStyle);QStage.$("hidden_sample_menu").updateStyle("display",
displayStyle)},sizeChanged:function(){this.stageWrapper.updateStyle("width",this.width);this.stageWrapper.updateStyle("height",this.height);this.stageWrapper.updateStyle("margin","{0}px 0px 0px {0}px".format(Config.Workspace.margin));if(this.startPage!=null)this.startPage.resize(this.width,this.height);this.textPanel.resize(this.width,this.height);this.promptDialog.resize(this.width,this.height);var objMgrLeft=this.width-Config.ObjectManager.marginRight-Config.ObjectManager.width;this.objectManager.moveWindowTo(objMgrLeft,
Config.ObjectManager.marginTop);this.resizeDrawingBoard(this.width,this.height);this.scaleplate.resize(this.width,this.height);this.slidePane.resize(this.width,this.height);if(this.scrimWrapper!=null){this.scrimWrapper.updateStyle("width",this.width+"px");this.scrimWrapper.updateStyle("height",this.height+"px");this.scrimWrapper.updateStyle("padding-top",this.height/2+"px")}},resizeDrawingBoard:function(workspaceWidth,workspaceHeight){var scaleplateSize=Config.General.showScale?Config.Scaleplate.size:
0;var boardWidth=workspaceWidth-scaleplateSize;var boardHeight=workspaceHeight-scaleplateSize;this.drawingBoard.resize(boardWidth,boardHeight)},transformPoint:function(point){return this.drawingBoard.transformPoint(point)},showGhost:function(d){return this.drawingBoard.showGhost(d)},getEventManager:function(owner){return this.eventManager.getRegistration(owner)},selectShapeById:function(id){this.painting.selectShapeById(id)},showScalePlate:function(show){this.scaleplate.setShowing(show);this.resizeDrawingBoard(this.width,
this.height);var scaleplateSize=Config.General.showScale?Config.Scaleplate.size:0;this.slidePane.setMargin(scaleplateSize,scaleplateSize)},showScrim:function(show,msg){this.scrimWrapper.updateStyle("display",show?null:"none");if(msg!=null)this.scrimWrapper.textContent=msg;else this.scrimWrapper.textContent=""},autoSave:function(){if(!this.autoSaveEnabled)return;if(!Config.General.offlineMode)return;var that=this;if(this.autoSaveTimer)clearTimeout(this.autoSaveTimer);this.autoSaveTimer=setTimeout(function(){try{that.saveToCache()}finally{that.autoSaveTimer=
null}},Config.Workspace.autoSaveDelay)},loadProject:function(id,callback){var that=this;if(Config.General.offlineMode===true){var cachedObj=QCache.getProject(id);if(cachedObj!=null){log("Loaded from cache with ID:",id);that.painting.deserialize(cachedObj,"JSON");that.slidePane.settingMenu.setProjectName(that.painting.name);if(typeof callback=="function")callback(true)}else{log("Project cache not found in cache with ID:",id);if(typeof callback=="function")callback(false)}}else{this.showScrim(true,
"Loading... Please Wait...");this.loadFromServer(id,function(obj){if(obj!=null){log("Loaded from server with ID:",id);that.painting.deserialize(obj,"JSON");that.slidePane.settingMenu.setProjectName(that.painting.name);if(typeof callback=="function")callback(true)}else{log("Nothing loaded from server with ID:",id);if(typeof callback=="function")callback(false)}that.showScrim(false)})}},loadWithUrlId:function(callback){var that=this;var urlId=QStage.getUrlHash("id");var shouldLoad=Config.General.offlineMode&&
urlId&&urlId.length>0||!Config.General.offlineMode&&QStage.isValidId(urlId);if(shouldLoad)this.loadProject(urlId,function(success){if(!success)that.autoSaveEnabled=true;if(typeof callback==="function")callback(success)});else{this.autoSaveEnabled=true;if(typeof callback==="function")callback(false)}},loadFromServer:function(id,callback){var that=this;var xmlhttp=new XMLHttpRequest;var url="http://stage.qlike.com/repository/?action=pull";var params="id="+id;xmlhttp.onreadystatechange=function(){if(xmlhttp.readyState==
4)if(typeof callback=="function")if(xmlhttp.status==200)try{var o=JSON.parse(xmlhttp.responseText);if(o.payload.length>0){var projectJson=o.payload[0].json_string;callback(JSON.parse(projectJson))}else callback(null)}catch(err){log(xmlhttp.responseText);callback(null)}else callback(null)};xmlhttp.open("POST",url,true);xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");xmlhttp.send(params)},sendToServer:function(callback){var that=this;this.showScrim(true,"Sending to server...");
