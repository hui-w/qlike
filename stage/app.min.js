...
for(var i=0;i<obj.payload.length;i++){var line=subContent.createChild("div",{class:"sub-item"});line.createChild("div",{class:"link",title:obj.payload[i].id,"data-id":obj.payload[i].id},obj.payload[i].id).addEventListener("click",function(){that.openProject(this.getAttribute("data-id"))},false)}if(obj.payload.length<=0)subContent.createChild("div",{class:"sub-item"},"No record")}catch(err$0){log(err$0)}},function(errorCode){log(errorCode)})}},renderAppIcon:function(wrapper){var icon=wrapper.createChildNS("svg",
