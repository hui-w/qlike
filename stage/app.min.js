...
display?"0px":"-{0}px".format(this.width));this.workspace.stageWrapper.updateStyle("left",display?"{0}px".format(this.width):"0px");if(display){var selected=document.querySelectorAll(".item-selected");for(var i=0;i<selected.length;i++)selected[i].removeClassName("item-selected")}},openProject:function(id){var that=this;if(id&&id.length>0){this.workspace.reset(true);QStage.setUrlHash("id",id);this.workspace.loadProject(id,function(loaded){that.setDisplay(false)})}else{this.workspace.newProject();this.setDisplay(false)}},
renderMenuItem:function(wrapper,subId){var that=this;var label="";var subLabel="";switch(subId){case 0:label="New";subLabel="New Project";break;case 1:label="Retrieve";subLabel="Retrieve from Server";break;case 2:label="Upload";subLabel="Upload Project File";break;case 3:label="Recent";subLabel=Config.General.offlineMode===true?"Recent Projects":"Recent Projects on Server";break}var item=wrapper.createChild("div",{class:"item","data-id":subId});var title=item.createChild("div",{class:"label no-select"},
label);title.addEventListener("click",function(){var item=this.parentNode;var items=item.parentNode.childNodes;for(var i=0;i<items.length;i++){if(items[i].nodeType!=1)continue;if(items[i].hasClassName("item-selected"))items[i].removeClassName("item-selected");else if(items[i]==item)items[i].addClassName("item-selected")}if(item.getAttribute("data-id")=="3")that.refreshProjectList()},false);var sub=item.createChild("div",{class:"sub"});sub.createChild("div",{class:"sub-label"},subLabel);var subContent=
sub.createChild("div",{class:"sub-content"});switch(subId){case 0:var line=subContent.createChild("div",{class:"sub-item"});line.createChild("a",{class:"link",href:"javascript:void(0)"},"Create a new project from scratch").addEventListener("click",function(){that.openProject()},false);break;case 1:var line=subContent.createChild("div",{class:"sub-item"});var form=line.createChild("form",{action:QStage.getAppUrl()});var txtId=form.createChild("input",{type:"text",class:"text",placeholder:"Project ID",
name:"id"});var btnGo=form.createChild("input",{type:"button",class:"button",value:"Retrieve"});btnGo.addEventListener("click",function(){if(QStage.isValidId(txtId.value))that.openProject(txtId.value)},false);break;case 2:if(Config.General.offlineMode===true){var line=subContent.createChild("div",{class:"sub-item"});var fileInput=line.createChild("input",{type:"file",class:"file",id:"fileToUpload",accept:".json"});var handleFileUpload=function(){var file=fileInput.files[0];if(!file)return;var reader=
...
try{var decodedPayload=atob(base64Payload);jsonObj=JSON.parse(decodedPayload)}catch(decodeError){throw new Error("Failed to decode file. Invalid format.");}}else throw new Error("File format is invalid. File is too short.");}if(jsonObj&&projectId){that.workspace.reset();QStage.setUrlHash("id",projectId);QCache.setProject(projectId,jsonObj);that.workspace.painting.deserialize(jsonObj,"JSON");that.setDisplay(false);fileInput.value=""}else throw new Error("Failed to parse project data.");}catch(error){alert("Error uploading file: "+
error.message);fileInput.value=""}};reader.onerror=function(){alert("Error reading file.");fileInput.value=""};reader.readAsText(file)};fileInput.addEventListener("change",handleFileUpload,false)}else{var line=subContent.createChild("form",{class:"sub-item",action:"http://stage.qlike.com/repository/upload.php",method:"post",enctype:"multipart/form-data"});line.createChild("input",{type:"file",class:"file",name:"fileToUpload",id:"fileToUpload",placeholder:"Project ID"});line.createChild("input",{type:"submit",
class:"button",value:"Upload",name:"submit"})}break;case 3:var line=subContent.createChild("div",{id:"project-list"});break}},refreshProjectList:function(){var that=this;var subContent=QStage.$("project-list");while(subContent.childNodes.length>0)subContent.removeChild(subContent.firstChild);subContent.createChild("div",{class:"sub-item"},"Loading the list...");if(Config.General.offlineMode===true)try{var projectIds=QCache.getAllProjectIds();projectIds.sort(function(a,b){var projectA=QCache.getProject(a);
var projectB=QCache.getProject(b);var timeA=projectA&&projectA.updatedAt?projectA.updatedAt:0;var timeB=projectB&&projectB.updatedAt?projectB.updatedAt:0;return timeB-timeA});while(subContent.childNodes.length>0)subContent.removeChild(subContent.firstChild);var activeProjects=[];var inactiveProjects=[];for(var i=0;i<projectIds.length;i++){var proj=QCache.getProject(projectIds[i]);if(!proj)continue;if(proj.active===false)inactiveProjects.push({id:projectIds[i],project:proj});else activeProjects.push({id:projectIds[i],
project:proj})}if(activeProjects.length>0)for(var i=0;i<activeProjects.length;i++){var projectId=activeProjects[i].id;var project=activeProjects[i].project;var line=subContent.createChild("div",{class:"sub-item",style:"display: flex; align-items: center; justify-content: space-between;"});var itemContainer=line.createChild("div",{style:"display: flex; flex-direction: column;"});var link=itemContainer.createChild("a",{class:"link",title:projectId,"data-id":projectId,href:"javascript:void(0)"},project.name?
project.name:"[ID#"+projectId+"]");link.addEventListener("click",function(){that.openProject(this.getAttribute("data-id"))},false);var createdDiv=itemContainer.createChild("div",{class:"last-updated"});createdDiv.textContent="Created: "+formatRelativeTime(parseInt(projectId));if(project.updatedAt){var lastUpdatedDiv=itemContainer.createChild("div",{class:"last-updated"});lastUpdatedDiv.textContent="Updated: "+formatRelativeTime(project.updatedAt)}var recycleBtn=line.createChild("button",{class:"button icon-button",
"data-id":projectId});var svg=document.createElementNS("http://www.w3.org/2000/svg","svg");svg.setAttribute("width","16");svg.setAttribute("height","16");svg.setAttribute("viewBox","0 0 16 16");svg.setAttribute("fill","currentColor");var path=document.createElementNS("http://www.w3.org/2000/svg","path");path.setAttribute("d","M6 2h4v1h3v1H2V3h3V2zM3 6h10l-1 8H4l-1-8zm2 1v6h1V7H5zm3 0v6h1V7H8z");svg.appendChild(path);recycleBtn.appendChild(svg);recycleBtn.addEventListener("click",function(e){e.stopPropagation();
var idToDeactivate=this.getAttribute("data-id");var project=QCache.getProject(idToDeactivate);if(project){project.active=false;if(QCache.setProject(idToDeactivate,project))that.refreshProjectList();else alert("Failed to delete project.")}},false)}else subContent.createChild("div",{class:"sub-item"},"No active project");if(inactiveProjects.length>0){subContent.createChild("div",{style:"font-size: 12px; color: #999; padding: 5px 0px; margin: 20px 0 10px 0;"},"Trash");var deleteAllBtnContainer=subContent.createChild("div",
{style:"margin-bottom: 10px;"});var deleteAllBtn=deleteAllBtnContainer.createChild("a",{class:"link",href:"javascript:void(0)"});deleteAllBtn.textContent="Empty Trash";deleteAllBtn.addEventListener("click",function(){if(confirm("Are you sure you want to permanently delete all trashed projects? This action cannot be undone.")){var allIds=QCache.getAllProjectIds();for(var i=0;i<allIds.length;i++){var proj=QCache.getProject(allIds[i]);if(proj&&proj.active===false)QCache.deleteProject(allIds[i])}that.refreshProjectList()}},
false);for(var k=0;k<inactiveProjects.length;k++){var projectId=inactiveProjects[k].id;var project=inactiveProjects[k].project;var line=subContent.createChild("div",{class:"sub-item",style:"display: flex; align-items: center; justify-content: space-between;"});var itemContainer=line.createChild("div",{style:"display: flex; flex-direction: column;"});itemContainer.createChild("span",{},project.name?project.name:"[ID#"+projectId+"]");var idDiv=itemContainer.createChild("div",{class:"last-updated"});
idDiv.textContent="ID: "+projectId;var restoreBtn=line.createChild("button",{class:"button icon-button","data-id":projectId});var restoreSvg=document.createElementNS("http://www.w3.org/2000/svg","svg");restoreSvg.setAttribute("width","16");restoreSvg.setAttribute("height","16");restoreSvg.setAttribute("viewBox","0 0 16 16");restoreSvg.setAttribute("fill","currentColor");var restorePath=document.createElementNS("http://www.w3.org/2000/svg","path");restorePath.setAttribute("d","M8 3a5 5 0 1 0 4.546 2.914l1.42-1.42A7 7 0 1 1 8 1v2zm3.5-1h-4v4h4V2z");
restoreSvg.appendChild(restorePath);restoreBtn.appendChild(restoreSvg);restoreBtn.addEventListener("click",function(e){e.stopPropagation();var idToRestore=this.getAttribute("data-id");var projectToRestore=QCache.getProject(idToRestore);if(projectToRestore){projectToRestore.active=true;if(QCache.setProject(idToRestore,projectToRestore))that.refreshProjectList();else alert("Failed to restore project.")}},false)}}}catch(err){log(err);while(subContent.childNodes.length>0)subContent.removeChild(subContent.firstChild);
...
obj.payload[i].id).addEventListener("click",function(){that.openProject(this.getAttribute("data-id"))},false)}if(obj.payload.length<=0)subContent.createChild("div",{class:"sub-item"},"No record")}catch(err$0){log(err$0)}},function(errorCode){log(errorCode)})}},renderAppIcon:function(wrapper){var icon=wrapper.createChildNS("svg",{width:32,height:32});var g=icon.createChildNS("g");var size=32;var padding=size*10/300;var path="M{0},{1} H{2} L {3},{4} H {0} Z".format(size/3+padding,padding,size*2/3,size-
padding,size/3-padding);g.createChildNS("path",{fill:"#22CB20",stroke:"#22CB20","stroke-width":padding,"stroke-linejoin":"round",d:path});g.createChildNS("path",{fill:"#06ACD4",stroke:"#06ACD4","stroke-width":padding,"stroke-linejoin":"round",d:path,transform:"translate(0,0), rotate({1},{0},{0})".format(size/2,1*90)});g.createChildNS("path",{fill:"#FFCC00",stroke:"#FFCC00","stroke-width":padding,"stroke-linejoin":"round",d:path,transform:"translate(0,0), rotate({1},{0},{0})".format(size/2,2*90)});
g.createChildNS("path",{fill:"#FA5151",stroke:"#FA5151","stroke-width":padding,"stroke-linejoin":"round",d:path,transform:"translate(0,0), rotate({1},{0},{0})".format(size/2,3*90)});return icon}};
