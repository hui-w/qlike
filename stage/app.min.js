...
id);targetFile+=id;location.href=targetFile});else{targetFile+=QStage.getUrlHash("id");location.href=targetFile}}});if(Config.General.developerMode==true){var exportButton=container.createChild("div",{class:"text-item no-select"},"Export ...");exportButton.addEventListener("click",function(e){var textPanel=that.getWorkspace().textPanel;textPanel.clearButtons();var btnSVG,btnCanvas;btnSVG=textPanel.buttonArea.createChild("div",{class:"button no-select selected"},"SVG");btnSVG.addEventListener("click",
function(e){btnSVG.addClassName("selected");btnCanvas.removeClassName("selected");var text=that.getWorkspace().painting.toSVG();textPanel.setText(text)});btnCanvas=textPanel.buttonArea.createChild("div",{class:"button no-select"},"Canvas");btnCanvas.addEventListener("click",function(e){btnSVG.removeClassName("selected");btnCanvas.addClassName("selected");var text=that.getWorkspace().painting.toCanvas();textPanel.setText(text)});var btnSelect=textPanel.buttonArea.createChild("div",{class:"button no-select"},
"Select All");btnSelect.addEventListener("click",function(e){textPanel.textArea.focus();textPanel.textArea.select()});var btnPreview=textPanel.buttonArea.createChild("div",{class:"button no-select"},"Preview");btnPreview.addEventListener("click",function(e){new_window=window.open("","","");new_window.document.write(textPanel.textArea.value);new_window.focus()});var text=that.getWorkspace().painting.toSVG();textPanel.setText(text);textPanel.show();console.log(text);that.pane.selectMenuItem(null)},
false)}},getWorkspace:function(){return this.pane.workspace},getPainting:function(){return this.pane.workspace},getDrawingBoard:function(){return this.pane.workspace.drawingBoard}};
...
setData:function(color,opacity,strokeWidth){this.icon.updateStyle("background-color",color);this.titleBackground.updateStyle("background-color",color);this.opacityLabel.textContent="{0}%".format(opacity*100);if(strokeWidth)this.widthLabel.textContent="{0}px".format(strokeWidth);this.codeInput.value=color;this.selectByValue(this.opacSelect,opacity);if(strokeWidth)this.selectByValue(this.widthSelect,strokeWidth)}};
...
"<rect";output+=' {0}="{1}"'.format("x",position.x);output+=' {0}="{1}"'.format("y",position.y);output+=' {0}="{1}"'.format("width",boundary.width);output+=' {0}="{1}"'.format("height",boundary.height);output+=' {0}="{1}"'.format("rotate",rotation);output+=' {0}="{1}"'.format("fill",QStage.brushToRgba(this.prop("fill")));output+=' {0}="{1}"'.format("stroke",QStage.brushToRgba(this.prop("stroke")));output+=' {0}="{1}"'.format("stroke-width",this.prop("stroke").width);output+=" />";return output}if(serializeType===
SerializeTypes.Canvas)return"[Canvas "+this.toString(SerializeTypes.TypeOnly)+"]";return null},render:function(){var that=this;this.shapeElement=this.svgGroup.createChildNS("path",{"data-label":"Shape",fill:QStage.brushToRgba(this.prop("fill")),stroke:QStage.brushToRgba(this.prop("stroke")),"stroke-width":this.prop("stroke").width,d:this.getPath()});this.renderComplete()}};var Rectangle=BaseShape.extend(rectangle_prototype);
