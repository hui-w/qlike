...
var ToolTypes={None:0,Picker:1,Move:2,Crop:4,Line:8,Rectangle:16,Ellipse:32,Polygon:64,Pencil:128,Pen:256,Label:512,Map:1024,Poi:2048,Block:4096,Eraser:8192};var SerializeTypes={TypeOnly:1,Abstract:2,JSON:4,SVG:8,Canvas:16};function log(){var now=(new Date).toISOString();var args=Array.prototype.slice.call(arguments);args.unshift("["+now+"]");console.log.apply(console,args)}
function clone(obj){if(typeof obj=="function"||Object(obj)!==obj)return obj;var res=new obj.constructor;for(var key in obj)if(obj.hasOwnProperty(key))res[key]=clone(obj[key]);return res}
...
if(result==null||result.length<1)return null;else return result[1]},alignToGrid:function(v){if(Config.General.snapToGrid)return Math.round(v/Config.General.gridSize)*Config.General.gridSize;else return v},generateNewProjectId:function(){if(Config.General.offlineMode===true)return Date.now().toString();else return"N"},brushToRgba:function(brush){return brush.color.toColorRgba(brush.opacity)},enhanceBoundary:function(boundary,forcePositive){var left=boundary.left;var top=boundary.top;var right=boundary.right;
var bottom=boundary.bottom;var width=right-left;var height=bottom-top;if(forcePositive&&width<0){width*=-1;left=right;right=left+width}if(forcePositive&&height<0){height*=-1;top=bottom;bottom=top+height}return{left:left,top:top,right:right,bottom:bottom,width:width,height:height,cx:left+width/2,cy:top+height/2,rx:width/2,ry:height/2}},supportsTransitions:function(){var b=document.body||document.documentElement,s=b.style,p="transition",e={Webkit:"webkitTransitionEnd",Moz:"transitionend",O:"oTransitionEnd otransitionend",
ms:"MSTransitionEnd",default:"transitionend"},v=["Moz","Webkit","O","ms"];if(typeof s[p]=="string")return{prefixedProperty:p,prefixedEventName:e["default"]};p=p.charAt(0).toUpperCase()+p.substr(1);for(var i=0;i<v.length;i++)if(typeof s[v[i]+p]=="string")return{prefixedProperty:"-"+v[i].toLowerCase()+"-"+p.toLowerCase(),prefixedEventName:e[v[i]]};return false},isLocal:function(){var protocol=document.location.protocol;return protocol!="https:"&&protocol!="http:"},getAppUrl:function(){var url="./";
if(QStage.isLocal())url+="index.html";return url},isValidId:function(id){return id!=null&&id.length==32},sendPostRequest:function(url,params,succCallback,errCallback){var xmlhttp=new XMLHttpRequest;xmlhttp.onreadystatechange=function(){if(xmlhttp.readyState==4)if(xmlhttp.status==200){if(typeof succCallback=="function")succCallback(xmlhttp.responseText)}else if(typeof errCallback=="function")errCallback(xmlhttp.status)};xmlhttp.open("POST",url,true);xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
xmlhttp.send(params)}});window.QStage=QStage})();String.prototype.format=function(){var args=arguments;return this.replace(/\{(\d+)\}/g,function(m,i){return args[i]})};String.prototype.trim=function(){return this.replace(/^(\s|\u00A0)+/,"").replace(/(\s|\u00A0)+$/,"")};Array.prototype.forEach=function(func){for(var i=0;i<this.length;i++){var item=this[i];if(typeof func=="function")func.call(this,item)}};
...
function formatRelativeTime(timestamp){if(!timestamp||typeof timestamp!=="number")return"Unknown";var now=Date.now();var diffMs=now-timestamp;if(diffMs<0)return"just now";var diffSeconds=Math.floor(diffMs/1E3);var diffMinutes=Math.floor(diffSeconds/60);var diffHours=Math.floor(diffMinutes/60);var diffDays=Math.floor(diffHours/24);var diffWeeks=Math.floor(diffDays/7);var diffMonths=Math.floor(diffDays/30);var diffYears=Math.floor(diffDays/365);if(diffSeconds<60)return diffSeconds<=0?"just now":diffSeconds+
" second"+(diffSeconds!==1?"s":"")+" ago";else if(diffMinutes<60)return diffMinutes+" minute"+(diffMinutes!==1?"s":"")+" ago";else if(diffHours<24)return diffHours+" hour"+(diffHours!==1?"s":"")+" ago";else if(diffDays<7)return diffDays+" day"+(diffDays!==1?"s":"")+" ago";else if(diffWeeks<4)return diffWeeks+" week"+(diffWeeks!==1?"s":"")+" ago";else if(diffMonths<12)return diffMonths+" month"+(diffMonths!==1?"s":"")+" ago";else return diffYears+" year"+(diffYears!==1?"s":"")+" ago"}
...
var Config={General:{developerMode:false,keyboardScrollSpeed:50,lockTool:false,snapToGrid:false,showScale:false,showObjectManager:false,gridSize:20,canvasBackground:"#CCCCCC",drawingBackground:"#FFFFFF",anchorTipNofified:false,offlineMode:true},LocalStorage:{mainKey:"q-stage",playerCacheKey:"player-cache"},Workspace:{margin:0,autoSaveDelay:800},SlidePane:{left:0,top:24},Scaleplate:{size:16,indicatorSize:5,indicatorColor:"RGBA(67, 74, 77, 0.5)"},ObjectManager:{width:240,marginTop:66,marginRight:20},
DrawingBoard:{fillColor:"#3F51B5",strokeColor:"#333333",fillOpacity:.8,strokeOpacity:1,strokeWidth:4},CanvasFrame:{marginLeft:8,marginTop:8,marginRight:8,marginBottom:12,borderDashArray:"1,0",borderColor:"#333333"},AnchorPoint:{width:10,height:10,fill:"RGBA(255, 255, 255, 0.2)",stroke:"RGBA(33, 33, 33, 1)",capturedFill:"#FE5722",capturedStroke:"#FE5722",highlightedFill:"#FFFFFF",highlightedStroke:"#FE5722",strokeWidth:1,selectedFill:"#ff0000",selectedStroke:"#FE5722",followingFill:"#FFFFFF",followingStroke:"#FE5722"},
BezierController:{size:6,fill:"RGBA(127, 127, 127, 1)",stroke:"RGBA(127, 127, 127, 1)",strokeWidth:1,capturedFill:"#ff0000",capturedStroke:"#FE5722",highlightedFill:"#FFFFFF",highlightedStroke:"#FE5722"},FrameHandle:{weight:4,length:16,borderSize:1,borderDashArray:"4,2",fill:"RGBA(255, 255, 255, 1)",stroke:"RGBA(33, 33, 33, 1)",capturedFill:"#FE5722",capturedStroke:"#FE5722",highlightedFill:"#FFFFFF",highlightedStroke:"#FE5722",ghostFill:"RGBA(0, 0, 0, 0)",ghostStroke:"RGBA(0, 0, 0, 0)"},Shape:{ghostWidth:40,
ghostStroke:"RGBA(255, 0, 0, 0)",ghostFill:"RGBA(255, 0, 0, 0)"}};
...
(function(){var QCache={extend:function(obj){for(var key in obj){if(!obj.hasOwnProperty(key))continue;this[key]=obj[key]}},payloadObject:{},currentVersion:"1.0.0"};QCache.extend({getVersion:function(){if(!this.payloadObject.hasOwnProperty("_version"))return null;return this.payloadObject._version},setVersion:function(version){this.payloadObject._version=version;return this.update(true)},checkAndUpgradeVersion:function(){var stored=this.getVersion();if(stored===null){log("QCache: no version found, setting to",
this.currentVersion);this.payloadObject._version=this.currentVersion;this.saveToCache(Config.LocalStorage.mainKey,JSON.stringify(this.payloadObject));return true}if(stored!==this.currentVersion){log("QCache: version mismatch. Stored:",stored,"Current:",this.currentVersion);if(this.performUpgrade(stored,this.currentVersion)){this.payloadObject._version=this.currentVersion;this.saveToCache(Config.LocalStorage.mainKey,JSON.stringify(this.payloadObject));log("QCache: upgrade successful.");return true}else{log("QCache: upgrade failed or cancelled.");
return false}}return true},performUpgrade:function(oldVersion,newVersion){log("QCache.performUpgrade: mock upgrade from",oldVersion,"to",newVersion);return true},get:function(key){if(!this.update(false))return null;if(this.payloadObject.hasOwnProperty(key))return this.payloadObject[key];else return null},set:function(key,value){this.payloadObject[key]=value;return this.update(true)},getProject:function(projectId){if(!this.update(false))return null;if(!this.payloadObject.hasOwnProperty("projects"))this.payloadObject.projects=
{};if(this.payloadObject.projects.hasOwnProperty(projectId))return this.payloadObject.projects[projectId];else return null},setProject:function(projectId,value,updateTimestamp){if(!this.payloadObject.hasOwnProperty("projects"))this.payloadObject.projects={};if(updateTimestamp===true)value.updatedAt=Date.now();else if(!value.updatedAt)value.updatedAt=Date.now();this.payloadObject.projects[projectId]=value;return this.update(true)},deleteProject:function(projectId){if(!this.update(false))return false;
if(!this.payloadObject.hasOwnProperty("projects"))return false;if(this.payloadObject.projects.hasOwnProperty(projectId)){delete this.payloadObject.projects[projectId];return this.update(true)}return false},getAllProjectIds:function(){if(!this.update(false))return[];if(!this.payloadObject.hasOwnProperty("projects"))return[];var projectIds=[];for(var key in this.payloadObject.projects)if(this.payloadObject.projects.hasOwnProperty(key))projectIds.push(key);return projectIds},isProjectNameExists:function(name){if(!this.update(false))return false;
if(!this.payloadObject.hasOwnProperty("projects"))return false;for(var key in this.payloadObject.projects)if(this.payloadObject.projects.hasOwnProperty(key)){var project=this.payloadObject.projects[key];if(project&&project.name===name)return true}return false},clear:function(){return this.clearCache(Config.LocalStorage.mainKey)},update:function(serialize){if(serialize){if(!this.payloadObject.hasOwnProperty("_version"))this.payloadObject._version=this.currentVersion;var saved=this.saveToCache(Config.LocalStorage.mainKey,
JSON.stringify(this.payloadObject));return saved}else try{var json=this.loadFromCache(Config.LocalStorage.mainKey);if(json!=null){this.payloadObject=JSON.parse(json);this.checkAndUpgradeVersion();return true}else return false}catch(err){log("Error:",err);return false}},saveToCache:function(key,str){if(!localStorage)return false;try{localStorage.setItem(key,str);return true}catch(e){log("Error saving to localStorage:",e);return false}},loadFromCache:function(key){if(!localStorage)return null;try{var str=
localStorage.getItem(key);return str}catch(e){log("Error loading from localStorage:",e);return null}},clearCache:function(key){if(!localStorage)return false;try{localStorage.removeItem(key);return true}catch(e){log("Error clearing localStorage:",e);return false}}});window.QCache=QCache})();function AppMenu(pane){this.pane=pane;this.width=200;this.title="Application"}
...
"Clear Project","ICON24_NEW",true,function(){that.getWorkspace().reset(true)});if(!Config.General.offlineMode)this.pane.renderIconButton(container,"Save","ICON24_SAVE",false,function(){if(!that.getWorkspace().needToBeSaved())return;that.getWorkspace().sendToServer(function(id){if(id==null)that.getWorkspace().drawingBoard.showMessage("Error: Not Saved");else{var urlId=QStage.getUrlHash("id");if(urlId==id)that.getWorkspace().drawingBoard.showMessage("Saved");else QStage.setUrlHash("id",id)}})});this.pane.renderIconButton(container,
"Play Animation ...","ICON24_PLAY",true,function(){if(Config.General.offlineMode===true){var jsonObj=that.getWorkspace().painting.serialize("JSON");var id=QStage.getUrlHash("id");if(localStorage){var cacheKey=Config.LocalStorage.mainKey;var cacheData={};try{var existingData=localStorage.getItem(cacheKey);if(existingData)cacheData=JSON.parse(existingData)}catch(e){}cacheData[Config.LocalStorage.playerCacheKey]=jsonObj;localStorage.setItem(cacheKey,JSON.stringify(cacheData))}var targetFile="./player/index.html";
...
promptDialog.setText("Getting sharing URL...");that.getWorkspace().sendToServer(function(id){QStage.setUrlHash("id",id);targetFile+=id;promptDialog.setText(targetFile);promptDialog.setEnabled(true);promptDialog.selectAll()})}else{targetFile+=QStage.getUrlHash("id");promptDialog.setText(targetFile);promptDialog.selectAll()}});if(QStage.isLocal()===true)this.pane.renderIconButton(container,"Export","ICON24_EXPORT",true,function(){try{var textPanel=that.getWorkspace().textPanel;textPanel.clearButtons();
var btnSVG=textPanel.buttonArea.createChild("div",{class:"button no-select selected"},"Export as SVG");var btnCanvas=textPanel.buttonArea.createChild("div",{class:"button no-select"},"Export as Canvas HTML");var btnSelect=textPanel.buttonArea.createChild("div",{class:"button no-select"},"Select All");var btnPreview=textPanel.buttonArea.createChild("div",{class:"button no-select"},"Preview");btnSVG.addEventListener("click",function(e){btnSVG.addClassName("selected");btnCanvas.removeClassName("selected");
var text=that.getWorkspace().painting.toSVG();textPanel.setText(text)});btnCanvas.addEventListener("click",function(e){btnSVG.removeClassName("selected");btnCanvas.addClassName("selected");var text=that.getWorkspace().painting.toCanvas();textPanel.setText(text)});btnSelect.addEventListener("click",function(e){textPanel.textArea.focus();textPanel.textArea.select()});btnPreview.addEventListener("click",function(e){var new_window=window.open("","","");new_window.document.write(textPanel.textArea.value);
new_window.focus()});var text=that.getWorkspace().painting.toSVG();textPanel.setText(text);textPanel.show();that.pane.selectMenuItem(null)}catch(e){that.getWorkspace().drawingBoard.showMessage("Export failed")}});this.pane.renderIconButton(container,"Save to Local","ICON24_DOWNLOAD",true,function(){if(Config.General.offlineMode===true){var jsonObj=that.getWorkspace().painting.serialize("JSON");var jsonString=JSON.stringify(jsonObj,null,2);var id=QStage.getUrlHash("id")||"project";var filename="project_"+
id+".json";var blob=new Blob([jsonString],{type:"application/json"});var url=window.URL.createObjectURL(blob);var link=document.createElement("a");link.href=url;link.download=filename;document.body.appendChild(link);link.click();document.body.removeChild(link);window.URL.revokeObjectURL(url)}else{var targetFile="http://stage.qlike.com/repository/?action=down&id=";if(that.getWorkspace().needToBeSaved())that.getWorkspace().sendToServer(function(id){QStage.setUrlHash("id",id);targetFile+=id;location.href=
targetFile});else{targetFile+=QStage.getUrlHash("id");location.href=targetFile}}})},getWorkspace:function(){return this.pane.workspace},getPainting:function(){return this.pane.workspace},getDrawingBoard:function(){return this.pane.workspace.drawingBoard}};
...
that.color)},false);var subBody=this.subContainer.createChild("div",{class:"sub-body"});this.renderPalette(subBody);this.button.addEventListener("click",function(){that.pane.selectMenuItem(this.parentNode)},false);if(this.isStroke){var color=this.getPainting().stroke.color;var opacity=this.getPainting().stroke.opacity;var strokeWidth=this.getPainting().stroke.width;this.setData(color,opacity,strokeWidth)}else{var color=this.getPainting().fill.color;var opacity=this.getPainting().fill.opacity;var strokeWidth=
null;this.setData(color,opacity,strokeWidth)}return menuItem},resize:function(width,height){var isSmallScreen=this.getWorkspace().isSmallScreen();var isLandscape=this.getWorkspace().isLandscape();var newScreenStatus=0;if(isSmallScreen)if(isLandscape)newScreenStatus=2;else newScreenStatus=1;if(this.screenStatus!=newScreenStatus){this.screenStatus=newScreenStatus;while(this.colorsPanel.childNodes.length>0)this.colorsPanel.removeChild(this.colorsPanel.firstChild);var colors=isSmallScreen?this.colors[1]:
this.colors[0];for(var i=0;i<colors.length;i++)this.renderColorButton(this.colorsPanel,colors[i],false);var width=this.width[0];if(isSmallScreen)if(isLandscape)width=this.width[2];else width=this.width[1];this.subContainer.updateStyle("width",width+"px")}},getWorkspace:function(){return this.pane.workspace},getPainting:function(){return this.pane.workspace.painting},renderPalette:function(subBody){var that=this;this.colorsPanel=subBody.createChild("div",{class:"colors"});subBody.createChild("div",
{class:"clear"});var attributesPanel=subBody.createChild("div",{class:"attributes"});var opacityPanel=attributesPanel.createChild("div",{class:"field"});opacityPanel.createChild("div",{class:"label"},"RGB Code:");this.codeInput=opacityPanel.createChild("input",{type:"text",class:"form-control",id:this.isStroke?"stroke-palette__code-input":"fill-palette__code-input",maxlength:7});attributesPanel.createChild("div",{class:"clear"});var opacityPanel=attributesPanel.createChild("div",{class:"field"});
opacityPanel.createChild("div",{class:"label"},"Color Opacity:");this.opacSelect=opacityPanel.createChild("select",{class:"form-control",id:this.isStroke?"stroke-palette__opacity-select":"fill-palette__opacity-select"});for(var i=0;i<=10;i++)this.opacSelect.createChild("option",{value:i/10},i*10+"%");attributesPanel.createChild("div",{class:"clear"});if(this.isStroke){this.widthPanel=attributesPanel.createChild("div",{class:"field"});this.widthPanel.createChild("div",{class:"label"},"Stroke Width:");
this.widthSelect=this.widthPanel.createChild("select",{class:"form-control",id:"width-select"});for(var i=0;i<=20;i++)this.widthSelect.createChild("option",{value:i}," "+i);attributesPanel.createChild("div",{class:"clear"})}},renderColorButton:function(panel,color){var that=this;var div=panel.createChild("div",{style:"background-color: "+color,title:color,"data-color":color});div.addEventListener("click",function(){var color=this.getAttribute("data-color");that.changeColor(color)},false)},changeColor:function(color){if(color==
null){this.pane.selectMenuItem(null);return}if(typeof this.onColorChanged=="function"){var opacity=this.getSelectValue(this.opacSelect);var strokeWidth=null;if(this.isStroke)strokeWidth=this.getSelectValue(this.widthSelect);this.onColorChanged(color,opacity,strokeWidth)}this.pane.selectMenuItem(null)},selectByValue:function(select,value){for(var i=0;i<select.options.length;i++)if(select.options[i].value==value)select.options[i].selected=true},getSelectValue:function(select){return select.options[select.selectedIndex].value},
setData:function(color,opacity,strokeWidth){this.icon.updateStyle("background-color",color);this.titleBackground.updateStyle("background-color",color);this.opacityLabel.textContent="{0}%".format(opacity*100);if(strokeWidth)this.widthLabel.textContent="{0}px".format(strokeWidth);this.codeInput.value=color;this.selectByValue(this.opacSelect,opacity);if(strokeWidth)this.selectByValue(this.widthSelect,strokeWidth)}};function DevMenu(pane){this.pane=pane;this.width=200;this.title="Developer Mode"}
DevMenu.prototype={renderInto:function(wrapper){var that=this;var menuItem=wrapper.createChild("div",{class:"menu-item dev-menu",title:this.title,id:"hidden_dev_menu",style:"display: none"});var button=menuItem.createChild("div",{});this.pane.render32pxSvgIcon(button,"ICON32_MENU_NONE");var subContainer=menuItem.createChild("div",{class:"sub-container",style:"width: {0}px;".format(this.width)});subContainer.createChild("div",{class:"sub-title no-select"},this.title);var subBody=subContainer.createChild("div",
{class:"sub-body"});var items=subBody.createChild("div",{class:"sub-items"});this.renderSubMenus(items);button.addEventListener("click",function(){that.pane.selectMenuItem(this.parentNode)},false);return menuItem},getWorkspace:function(){return this.pane.workspace},getPainting:function(){return this.pane.workspace.painting},renderSubMenus:function(container){var that=this;var rBlack=null;var rRed=null;var rBlue=null;var redCenter=null;var blueCenter=null;var position={x:200,y:150};var boundary={left:100,
top:80,right:500,bottom:250};boundary=QStage.enhanceBoundary(boundary);var angle=40;var dcx=90;var dcy=120;function getRotationOffset(cx1,cy1,cx2,cy2,angle){var d=angle/180*Math.PI;var dx=cx2-cx1;var dy=cy2-cy1;var ddx=dx*(1-Math.cos(d))-dy*Math.sin(d);var ddy=dx*Math.sin(d)+dy*(1-Math.cos(d));return{x:ddx,y:ddy}}var tool1=container.createChild("div",{class:"text-item no-select"},"Rotate Sample");var tool2=container.createChild("div",{class:"text-item no-select"},"Move Blue Rect");tool1.addEventListener("click",
function(){var svg=that.getWorkspace().drawingBoard.underGroup;if(rBlack==null)rBlack=svg.createChildNS("rect",{x:boundary.left,y:boundary.top,width:boundary.width,height:boundary.height,fill:"none",stroke:"RGBA(0, 0, 0, 0.5)","stroke-width":1});if(rRed==null){rRed=svg.createChildNS("rect",{x:boundary.left,y:boundary.top,width:boundary.width,height:boundary.height,fill:"RGBA(255, 0, 0, 0.5)",stroke:"none"});redCenter=svg.createChildNS("circle",{cx:boundary.cx,cy:boundary.cy,r:10,fill:"none",stroke:"RGBA(255, 0, 0, 1)",
"stroke-width":1})}if(rBlue==null){rBlue=svg.createChildNS("rect",{x:boundary.left,y:boundary.top,width:boundary.width,height:boundary.height,fill:"RGBA(0, 0, 255, 0.5)",stroke:"none"});blueCenter=svg.createChildNS("circle",{cx:boundary.cx+dcx,cy:boundary.cy+dcy,r:10,fill:"none",stroke:"RGBA(0, 0, 255, 1)","stroke-width":1})}var blackTransform="translate({0}, {1})".format(position.x,position.y);rBlack.setAttribute("transform",blackTransform);var redTransform="translate({3}, {4}) rotate({0}, {1}, {2})".format(angle,
boundary.cx,boundary.cy,position.x,position.y);rRed.setAttribute("transform",redTransform);redCenter.setAttribute("transform","translate({0}, {1})".format(position.x,position.y));var blueTransform="translate({3}, {4}) rotate({0}, {1}, {2})".format(angle,boundary.cx+dcx,boundary.cy+dcy,position.x,position.y);rBlue.setAttribute("transform",blueTransform);blueCenter.setAttribute("transform","translate({0}, {1})".format(position.x,position.y));tool2.updateStyle("display","block")},false);tool2.addEventListener("click",
function(){var delta=that.getRotationOffset(boundary.cx,boundary.cy,boundary.cx+dcx,boundary.cy+dcy,angle);var blueTransform="translate({3}, {4}) rotate({0}, {1}, {2})".format(angle,boundary.cx+dcx,boundary.cy+dcy,position.x+delta.x,position.y+delta.y);rBlue.setAttribute("transform",blueTransform)},false);var tool3=container.createChild("div",{class:"text-item no-select"},"Animation Stage");var stage=null;tool3.addEventListener("click",function(){if(stage==null){var addIcon=function(){stage.addIcon();
if(stage.icons.length<3)setTimeout(addIcon,1E3)};var containerGroup=that.getWorkspace().drawingBoard.underGroup;var svgWidth=that.getWorkspace().drawingBoard.svgSize.width;var svgHeight=that.getWorkspace().drawingBoard.svgSize.height;stage=new Stage(containerGroup,svgWidth,svgHeight);addIcon()}else{stage.destroy();stage=null}},false);var serializeButton=container.createChild("div",{class:"text-item no-select"},"Server - List All");serializeButton.addEventListener("click",function(e){that.request("http://stage.qlike.com/repository/?action=pull",
null,function(response){var object=JSON.parse(response);console.log(object.payload.length+" Record(s):");for(var i=0;i<object.payload.length;i++)console.log("- "+i+" "+object.payload[i].id)});that.pane.selectMenuItem(null)},false);var deserializeButton=container.createChild("div",{class:"text-item no-select"},"Server - Clear");deserializeButton.addEventListener("click",function(e){that.request("http://stage.qlike.com/repository/?action=del",null,function(response){console.log(response)});that.pane.selectMenuItem(null)},
false);var clearButton=container.createChild("div",{class:"text-item no-select"},"Server - Prepare");clearButton.addEventListener("click",function(e){var params="payload="+JSON.stringify(GET_SAMPLE_OBJ());that.request("http://stage.qlike.com/repository/?action=push",params,function(response){console.log(response)});that.pane.selectMenuItem(null)},false);var exportDev=container.createChild("div",{class:"text-item no-select"},"Export");exportDev.addEventListener("click",function(e){var textPanel=that.getWorkspace().textPanel;
textPanel.clearButtons();var btnSVG=textPanel.buttonArea.createChild("div",{class:"button no-select selected"},"Export as SVG");var btnCanvas=textPanel.buttonArea.createChild("div",{class:"button no-select"},"Export as Canvas HTML");var btnSelect=textPanel.buttonArea.createChild("div",{class:"button no-select"},"Select All");var btnPreview=textPanel.buttonArea.createChild("div",{class:"button no-select"},"Preview");btnSVG.addEventListener("click",function(){btnSVG.addClassName("selected");btnCanvas.removeClassName("selected");
var text=that.getWorkspace().painting.toSVG();textPanel.setText(text)});btnCanvas.addEventListener("click",function(){btnSVG.removeClassName("selected");btnCanvas.addClassName("selected");var text=that.getWorkspace().painting.toCanvas();textPanel.setText(text)});btnSelect.addEventListener("click",function(){textPanel.textArea.focus();textPanel.textArea.select()});btnPreview.addEventListener("click",function(){var new_window=window.open("","","");new_window.document.write(textPanel.textArea.value);
new_window.focus()});var text=that.getWorkspace().painting.toSVG();textPanel.setText(text);textPanel.show();that.pane.selectMenuItem(null)},false);var renderSampleBtn=container.createChild("div",{class:"text-item no-select"},"Render Sample PNG");renderSampleBtn.addEventListener("click",function(e){that.renderSample(that.getDrawingBoard().shapeGroup);that.pane.selectMenuItem(null)},false);var getSampleBtn=container.createChild("div",{class:"text-item no-select"},"Get Sample JSON");getSampleBtn.addEventListener("click",
function(e){var jsonObj=that.getWorkspace().painting.serialize("STRING");console.log(jsonObj);that.pane.selectMenuItem(null)},false)},getWorkspace:function(){return this.pane.workspace},getDrawingBoard:function(){return this.pane.workspace.drawingBoard},getDeltaAngle:function(p0,p1,p2){var alpha0=Math.atan2(p1.x-p0.x,p1.y-p0.y);var alpha1=Math.atan2(p2.x-p0.x,p2.y-p0.y);return alpha0-alpha1},getRotationOffset:function(cx1,cy1,cx2,cy2,angle){var d=angle/180*Math.PI;var cos=Math.cos(d);var dx=cx2-cx1;
var dy=cy2-cy1;var r=Math.sqrt(Math.pow(dx,2)+Math.pow(dy,2));var a=Math.pow(r,2);var b,c,resX1,resX2,resY1,resY2;if(cx1==cx2){b=-2*Math.pow(r,2)*(cx1*cos-cx2*cos+cx2);c=-Math.pow(r*(cy1-cy2),2)+Math.pow(r,4)*Math.pow(cos,2)+2*Math.pow(r,2)*cos*(cx1-cx2)*cx2+Math.pow(r*cx2,2);resX1=(-b+Math.sqrt(Math.pow(b,2)-4*a*c))/(2*a);resX2=(-b-Math.sqrt(Math.pow(b,2)-4*a*c))/(2*a);resY1=(Math.pow(r,2)*cos+(cx1-cx2)*(cx2-resX1))/(cy1-cy2)+cy2;resY2=(Math.pow(r,2)*cos+(cx1-cx2)*(cx2-resX2))/(cy1-cy2)+cy2}else{b=
-2*Math.pow(r,2)*(cy1*cos-cy2*cos+cy2);c=-Math.pow(r*(cx1-cx2),2)+Math.pow(r,4)*Math.pow(cos,2)+2*Math.pow(r,2)*cos*(cy1-cy2)*cy2+Math.pow(r*cy2,2);resY1=(-b+Math.sqrt(Math.pow(b,2)-4*a*c))/(2*a);resY2=(-b-Math.sqrt(Math.pow(b,2)-4*a*c))/(2*a);resX1=(Math.pow(r,2)*cos+(cy1-cy2)*(cy2-resY1))/(cx1-cx2)+cx2;resX2=(Math.pow(r,2)*cos+(cy1-cy2)*(cy2-resY2))/(cx1-cx2)+cx2}var refAngle=this.getDeltaAngle({x:cx2,y:cy2},{x:cx1,y:cy1},{x:resX1,y:resY1})*Math.sin(d);var offset=null;if(refAngle>0)offset={x:cx1-
resX1,y:cy1-resY1};else offset={x:cx1-resX2,y:cy1-resY2};return offset},request:function(url,params,callback){var that=this;var xmlhttp=new XMLHttpRequest;xmlhttp.onreadystatechange=function(){if(xmlhttp.readyState==4)if(typeof callback=="function")if(xmlhttp.status==200)callback(xmlhttp.responseText);else callback(null)};xmlhttp.open("POST",url,true);xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");xmlhttp.send(params)},renderSample:function(wrapper){var g=wrapper.createChildNS("g",
{transform:"translate(100.000000,967.000000) scale(0.100000,-0.100000)"});var d="";d+="M2569.316,8420.336c-399.541-31.997-747.573-145.933-1050.356-344.917c-458.848-302.002-738.218-738.218-795.962-1242.329 ";d+="c-11.704-101.45-5.459-313.711,12.485-410.469l12.485-70.229l-51.499-101.45c-56.191-110.806-124.858-280.151-175.581-436.22 ";d+="l-33.56-101.44l-53.848-25.752c-67.891-32.778-149.038-110.811-184.16-177.92c-70.234-134.224-63.208-301.226,18.73-429.199 ";d+="c74.917-118.608,203.672-190.405,342.578-190.405h63.989l6.24-87.402c6.24-108.477,21.074-198.213,48.384-294.199l20.283-73.35 ";
d+="l-22.627-15.605c-56.182-39.014-127.197-122.515-159.971-188.843c-28.096-56.973-33.555-74.922-36.675-127.207 ";d+="c-2.349-48.379,0-68.672,14.043-104.57c43.701-116.27,177.144-202.891,401.885-259.844 ";d+="c173.242-43.711,273.906-53.848,534.541-53.848c255.181,0,369.888,10.137,570.44,49.941 ";d+="c305.127,60.859,402.666,125.645,428.418,282.48c13.262,85.059-11.704,216.172-56.187,295.762 ";d+="c-15.605,27.31-15.605,30.425-3.12,67.114c33.555,101.445,130.313,216.934,219.277,260.635 ";d+="c43.696,21.855,56.187,24.189,99.888,21.064c40.581-2.334,55.405-7.793,83.496-27.305c57.749-42.92,124.858-147.49,167.778-262.197 ";
d+="l14.043-36.689l-28.872-62.427c-40.576-88.965-53.843-152.949-49.941-241.914c3.906-91.289,27.314-144.355,88.179-202.109 ";d+="c99.106-93.633,231.772-130.313,581.367-159.961c391.738-34.336,726.513-19.512,963.74,42.129 ";d+="c92.08,24.199,210.693,84.277,258.291,131.094c60.879,59.316,77.266,99.883,77.266,184.961c0,64.746-2.344,74.902-27.314,124.844 ";d+="c-32.773,66.348-124.082,160.762-187.285,194.321c-23.418,12.48-43.701,23.408-45.264,24.189 ";d+="c-0.781,0.771,3.906,24.971,10.137,53.838c18.74,80.381,34.346,204.453,39.814,314.492l5.449,99.873l90.537,0.791 ";
d+="c107.676,0,154.502,14.043,237.998,71.006c268.447,183.389,234.111,600.093-60.879,736.655l-56.953,26.533l-53.066,136.563 ";d+="c-28.877,74.917-82.715,205.239-118.613,288.735c-97.539,223.96-91.309,201.328-73.35,276.25 ";d+="c43.701,179.473,55.4,427.632,28.086,609.453c-64.766,437.783-309.805,813.911-702.314,1076.108 ";d+="c-280.146,188.066-613.359,305.122-991.055,348.042C2915.019,8417.992,2633.315,8425.019,2569.316,8420.336z M3025.053,8367.27 ";d+="c246.587-32.778,527.515-114.712,718.696-211.479c234.893-118.613,460.41-297.314,601.65-479.922 ";
d+="c230.996-295.752,342.588-661.733,312.939-1024.6c-9.365-120.957-31.221-252.056-40.586-241.914 ";d+="c-3.115,3.125-13.271,28.877-23.418,56.968c-51.494,147.485-166.201,336.333-283.262,468.213 ";d+="c-278.584,312.144-710.117,526.738-1191.601,593.071l-67.109,8.584l-24.199,52.28c-60.864,130.322-177.139,204.453-321.509,204.453 ";d+="c-156.846,0-280.918-85.84-333.208-229.419l-16.382-46.826l-69.458-13.267C1700.005,7390.258,1209.165,7065.634,932.915,6606 ";d+="c-26.533-44.473-53.843-85.054-60.869-89.736c-6.24-5.459-28.872-32.773-50.723-60.869c-24.976-32.773-40.581-47.598-43.701-41.362 ";
d+="c-14.824,26.538-24.189,172.461-20.288,309.023c3.125,113.149,8.584,166.221,24.971,242.695 ";d+="c53.062,254.395,165.439,490.059,333.989,698.418c63.994,79.6,216.943,227.861,298.1,290.293 ";d+="c322.285,245.029,714.024,392.515,1131.514,426.851C2629.409,8387.563,2932.182,8378.979,3025.053,8367.27z M2803.427,7740.644 ";d+="c145.142-45.259,241.133-200.552,214.595-347.261c-25.747-139.688-125.63-239.57-266.094-264.541 ";d+="c-105.347-18.726-227.871,30.43-294.976,120.171c-132.666,174.805-54.624,426.079,151.387,491.631 ";
d+="C2679.35,7763.275,2730.854,7763.275,2803.427,7740.644z M3149.121,7483.905c703.887-112.373,1236.084-513.472,1421.816-1070.649 ";d+="c65.547-196.646,82.715-353.5,74.902-688.276c-3.105-122.515-8.574-274.683-12.471-339.448 ";d+="c-5.469-91.309-4.688-142.808,4.668-234.111c9.365-85.84,11.709-169.336,8.594-316.045c-3.125-199.766-12.49-291.074-42.93-421.396 ";d+="l-13.252-58.525l-44.492-17.168c-56.182-22.622-236.436-58.521-427.637-85.825c-138.115-19.512-173.232-21.074-386.269-21.855 ";d+="c-189.629,0-251.279,3.125-323.848,14.043c-109.253,17.949-245.034,52.285-313.706,79.59l-49.937,20.298l-29.658,71.006 ";
d+="c-38.237,94.424-82.72,168.564-128.76,216.172l-38.232,39.014l78.813,2.344c73.359,2.324,79.6,3.115,79.6,17.939 ";d+="c0,15.605-4.683,15.605-245.82,13.271c-280.923-1.563-323.843-5.469-323.843-28.887c0-14.824,4.683-15.596,78.032-13.262 ";d+="l78.042,2.344l-23.418-17.949c-80.371-60.869-142.021-144.365-179.478-239.57c-20.293-50.723-24.194-56.172-49.941-63.213 ";d+="c-372.231-99.097-817.031-112.358-1200.967-34.331c-193.535,39.019-326.191,94.419-351.157,145.923 ";d+="c-19.517,40.586-54.629,195.879-63.999,285.625c-13.267,117.051-6.235,250.488,20.293,384.707l21.064,106.914l-16.382,88.955 ";
d+="c-28.091,148.267-35.117,240.347-30.435,395.645c5.459,152.944,17.168,251.274,48.384,378.471 ";d+="c162.314,678.13,702.319,1180.674,1455.361,1354.697c139.687,31.997,130.322,35.112,134.228-41.357 ";d+="c3.115-53.066,8.574-74.917,29.648-119.399c51.504-107.686,161.538-188.062,268.442-196.65l51.499-4.678v-168.555 ";d+="c0-92.866,3.125-173.247,7.031-179.487c3.901-6.24,12.48-8.584,19.512-6.24c10.918,4.683,12.48,26.533,12.48,180.264v174.795 ";d+="l34.336,4.683c92.856,14.048,199.771,98.33,246.592,195.088c24.185,48.389,27.31,63.213,29.653,131.104 ";
d+="c2.339,42.139,6.24,76.475,10.142,76.475C3069.521,7496.391,3106.987,7490.927,3149.121,7483.905z M790.107,6310.248 ";d+="c-53.843-137.339-89.741-309.018-109.248-523.613l-4.683-52.29l-67.891-3.12c-37.456-2.339-69.453-3.12-71.016-1.563 ";d+="c-3.896,3.901,53.066,172.461,98.33,287.954c53.843,137.338,161.533,354.282,170.898,344.917 ";d+="C808.057,6360.976,801.03,6337.558,790.107,6310.248z M4773.056,5893.535c42.91-109.248,78.809-199.771,78.809-201.328 ";d+="c0-1.563-39.014-3.12-86.621-3.901l-87.402-0.776l4.688,98.32c2.344,54.624,2.344,150.61-0.781,214.6 ";
d+="c-3.906,85.84-3.125,112.373,3.125,103.007C4690.322,6096.43,4729.345,6002.006,4773.056,5893.535z M679.297,5581.391 ";d+="c3.12-61.641,14.829-165.43,25.752-230.977l19.512-119.399l-16.387-82.72c-8.589-45.259-18.73-112.368-21.851-149.043 ";d+="l-5.464-66.328l-28.872-7.031c-38.242-9.355-135.781,9.365-196.65,38.242c-126.421,59.307-215.376,202.114-215.376,343.354 ";d+="c0,109.253,35.894,195.869,113.149,273.13c74.131,74.131,162.314,110.811,266.099,112.368l55.41,0.776L679.297,5581.391z ";d+=" M4854.208,5643.046c251.27-53.062,382.373-330.093,266.094-559.517c-44.482-87.402-150.615-172.461-248.154-198.203 ";
d+="c-42.139-11.709-158.408-14.834-173.232-4.688c-5.469,3.115-10.928,53.848-14.043,130.313 ";d+="c-3.125,68.677-8.594,149.834-12.49,179.492c-7.031,48.379,3.115,418.267,11.699,446.362 ";d+="C4689.55,5654.75,4782.412,5658.652,4854.208,5643.046z M821.323,4371.069c169.336-101.44,623.501-167.788,995.737-145.933 ";d+="c171.675,10.918,312.925,29.648,444.023,60.078c49.155,11.719,96.763,21.855,105.347,22.656 ";d+="c33.55,2.324,78.032-138.125,77.251-241.152c-1.553-167.773-91.299-231.758-412.807-296.543 ";d+=
"c-195.874-39.004-287.173-46.816-550.151-46.816c-269.224,0-341.792,7.031-515.81,50.742 ";d+="c-165.439,41.348-278.589,96.758-341.802,169.336c-80.381,91.289-71.792,220.059,24.976,344.121 ";d+="c30.435,39.805,108.472,110.815,122.515,110.815C773.726,4398.374,796.348,4385.893,821.323,4371.069z M4685.644,4296.152 ";d+="c95.986-62.422,166.221-164.648,172.451-251.27c5.479-67.891-10.137-109.258-57.734-161.543 ";d+="c-94.434-101.445-290.293-163.086-600.088-188.066c-248.164-20.293-771.777,12.5-959.848,60.078 ";
d+="c-71.006,18.75-154.507,60.098-195.088,97.559c-88.179,81.152-107.686,197.422-57.744,344.922 ";d+="c25.752,77.246,45.264,109.238,60.083,102.227c78.037-35.117,217.725-71.016,360.527-92.871 ";d+="c113.926-17.168,384.707-24.18,523.613-12.48c262.207,21.855,581.377,79.59,654.726,117.832 ";d+="C4624.775,4333.608,4631.025,4332.055,4685.644,4296.152z ";g.createChildNS("path",{d:d,fill:"RGBA(0,0,0,0.2)"})}};function DevMenu(pane){this.pane=pane;this.width=200;this.title="Developer Mode"}
DevMenu.prototype={renderInto:function(wrapper){var that=this;var menuItem=wrapper.createChild("div",{class:"menu-item dev-menu",title:this.title,id:"hidden_dev_menu",style:"display: none"});var button=menuItem.createChild("div",{});this.pane.render32pxSvgIcon(button,"ICON32_MENU_NONE");var subContainer=menuItem.createChild("div",{class:"sub-container",style:"width: {0}px;".format(this.width)});subContainer.createChild("div",{class:"sub-title no-select"},this.title);var subBody=subContainer.createChild("div",
{class:"sub-body"});var items=subBody.createChild("div",{class:"sub-items"});this.renderSubMenus(items);button.addEventListener("click",function(){that.pane.selectMenuItem(this.parentNode)},false);return menuItem},getWorkspace:function(){return this.pane.workspace},getPainting:function(){return this.pane.workspace.painting},renderSubMenus:function(container){var that=this;var rBlack=null;var rRed=null;var rBlue=null;var redCenter=null;var blueCenter=null;var position={x:200,y:150};var boundary={left:100,
top:80,right:500,bottom:250};boundary=QStage.enhanceBoundary(boundary);var angle=40;var dcx=90;var dcy=120;function getRotationOffset(cx1,cy1,cx2,cy2,angle){var d=angle/180*Math.PI;var dx=cx2-cx1;var dy=cy2-cy1;var ddx=dx*(1-Math.cos(d))-dy*Math.sin(d);var ddy=dx*Math.sin(d)+dy*(1-Math.cos(d));return{x:ddx,y:ddy}}var tool1=container.createChild("div",{class:"text-item no-select"},"Rotate Sample");var tool2=container.createChild("div",{class:"text-item no-select"},"Move Blue Rect");tool1.addEventListener("click",
function(){var svg=that.getWorkspace().drawingBoard.underGroup;if(rBlack==null)rBlack=svg.createChildNS("rect",{x:boundary.left,y:boundary.top,width:boundary.width,height:boundary.height,fill:"none",stroke:"RGBA(0, 0, 0, 0.5)","stroke-width":1});if(rRed==null){rRed=svg.createChildNS("rect",{x:boundary.left,y:boundary.top,width:boundary.width,height:boundary.height,fill:"RGBA(255, 0, 0, 0.5)",stroke:"none"});redCenter=svg.createChildNS("circle",{cx:boundary.cx,cy:boundary.cy,r:10,fill:"none",stroke:"RGBA(255, 0, 0, 1)",
...
if(stage.icons.length<3)setTimeout(addIcon,1E3)};var containerGroup=that.getWorkspace().drawingBoard.underGroup;var svgWidth=that.getWorkspace().drawingBoard.svgSize.width;var svgHeight=that.getWorkspace().drawingBoard.svgSize.height;stage=new Stage(containerGroup,svgWidth,svgHeight);addIcon()}else{stage.destroy();stage=null}},false);var serializeButton=container.createChild("div",{class:"text-item no-select"},"Server - List All");serializeButton.addEventListener("click",function(e){that.request("http://stage.qlike.com/repository/?action=pull",
null,function(response){var object=JSON.parse(response);console.log(object.payload.length+" Record(s):");for(var i=0;i<object.payload.length;i++)console.log("- "+i+" "+object.payload[i].id)});that.pane.selectMenuItem(null)},false);var deserializeButton=container.createChild("div",{class:"text-item no-select"},"Server - Clear");deserializeButton.addEventListener("click",function(e){that.request("http://stage.qlike.com/repository/?action=del",null,function(response){console.log(response)});that.pane.selectMenuItem(null)},
false);var clearButton=container.createChild("div",{class:"text-item no-select"},"Server - Prepare");clearButton.addEventListener("click",function(e){var params="payload="+JSON.stringify(GET_SAMPLE_OBJ());that.request("http://stage.qlike.com/repository/?action=push",params,function(response){console.log(response)});that.pane.selectMenuItem(null)},false)}};var DevMenu=DevMenu;
...
g.createChildNS("rect",{"x":buttonSize/2+.5,"y":buttonSize/2+.5,"width":buttonSize/2-1,"height":buttonSize/2-1,"stroke":"#333333","stroke-width":1,"fill":"#ffffff"});break}}};function SettingMenu(pane){this.pane=pane;this.width=200;this.title="Settings"}
...
var settings=subBody.createChild("div",{class:"sub-items"});var btnLock=this.renderCheckBox(settings,"Lock Selected Tool","Config.General.lockTool",null);var stage=null;var clickCount=0;var beginInitCount=6;var maxInitCount=3;btnLock.addEventListener("click",function(){clickCount++;console.log(clickCount);if(clickCount>=beginInitCount+maxInitCount){if(stage!=null){that.getWorkspace().switchDeveloperMode(false);that.getWorkspace().drawingBoard.showMessage("Cancel developer mode");stage.destroy();stage=
null}clickCount=0;that.pane.selectMenuItem(null)}if(clickCount==beginInitCount-1){that.getWorkspace().switchDeveloperMode(true);that.getWorkspace().drawingBoard.showMessage("Start developer mode")}if(clickCount>=beginInitCount){var addIcon=function(){stage.addIcon();if(stage.icons.length<2)setTimeout(addIcon,1E3)};if(stage==null){var containerGroup=that.getWorkspace().drawingBoard.underGroup;var svgWidth=that.getWorkspace().drawingBoard.svgSize.width;var svgHeight=that.getWorkspace().drawingBoard.svgSize.height;
stage=new Stage(containerGroup,svgWidth,svgHeight)}addIcon()}},false);this.renderCheckBox(settings,"Snap to Grids","Config.General.snapToGrid",null);this.renderCheckBox(settings,"Show Scale Rulers","Config.General.showScale",function(){that.getWorkspace().showScalePlate(Config.General.showScale)});this.renderCheckBox(settings,"Objects Manager","Config.General.showObjectManager",function(){that.getWorkspace().objectManager.show(Config.General.showObjectManager)});this.pane.addSeperator(settings,"Project Name");
var field=settings.createChild("div",{class:"field no-select flex-fill"});var input=field.createChild("input",{id:"project-name-input",type:"text",class:"text-input flex-fill",style:"padding: 8px 6px; margin: 0 10px;",value:this.getWorkspace().painting.name});input.addEventListener("change",function(){that.getWorkspace().painting.name=this.value;that.getWorkspace().autoSave();var newName=this.value?this.value:"Empty";that.getWorkspace().drawingBoard.showMessage("Project name updated to "+newName)});
this.nameInput=input;button.addEventListener("click",function(){that.pane.selectMenuItem(this.parentNode)},false);return menuItem},setProjectName:function(text){this.nameInput.value=text},getWorkspace:function(){return this.pane.workspace},getDrawingBoard:function(){return this.pane.workspace.drawingBoard},renderMenuItem:function(wrapper,label,configKey,callback){var button=wrapper.createChild("div",{class:"text-item no-select"},label);button.addEventListener("click",function(e){if(this.hasClassName("selected")){this.removeClassName("selected");
eval(configKey+" = false")}else{this.addClassName("selected");eval(configKey+" = true")}if(typeof callback=="function")callback()},false);if(eval(configKey))button.addClassName("selected")},renderCheckBox:function(wrapper,label,configKey,callback){var button=wrapper.createChild("div",{class:"checkbox-item no-select"},label);var icon=button.createChildNS("svg",{class:"icon-checkbox",width:24,height:24});var gChecked=icon.createChildNS("g",{class:"icon-group-checked"});gChecked.createChildNS("path",
{fill:"#ffffff",d:"M6.534,9.467l-1.867,1.867l6,6L24,4l-1.865-1.867L10.667,13.6L6.534,9.467z M21.334,21.333H2.667V2.667H16V0H2.667C1.201,0,0,1.2,0,2.667v18.667C0,22.8,1.201,24,2.667,24h18.667C22.801,24,24,22.8,24,21.333V10.667h-2.666V21.333z"});var gNormal=icon.createChildNS("g",{class:"icon-group-normal"});gNormal.createChildNS("path",{fill:"#010101",d:"M21.333,2.667v18.667H2.667V2.667H21.333 M21.333,0H2.667C1.2,0,0,1.201,0,2.667v18.667C0,22.801,1.2,24,2.667,24h18.667C22.8,24,24,22.801,24,21.334V2.667C24,1.201,22.8,0,21.333,0L21.333,0z"});
button.addEventListener("click",function(e){if(this.hasClassName("selected")){this.removeClassName("selected");eval(configKey+" = false")}else{this.addClassName("selected");eval(configKey+" = true")}if(typeof callback=="function")callback()},false);if(eval(configKey))button.addClassName("selected");return button}};
function SlidePane(workspace){this.workspace=workspace;this.appMenu=null;this.toolMenu=null;this.fillMenu=null;this.strokeMenu=null;this.settingMenu=null;this.propertyMenu=null;this.wrapper=null;this.menuDoms=[];this.selectedMenuDom=null}
...
this.strokeMenu=new ColorMenu(this,true);this.menuDoms.push(this.strokeMenu.renderInto(this.wrapper));this.settingMenu=new SettingMenu(this);this.menuDoms.push(this.settingMenu.renderInto(this.wrapper));this.propertyMenu=new PropertyMenu(this);this.menuDoms.push(this.propertyMenu.renderInto(this.wrapper));var devMenu=new DevMenu(this);this.menuDoms.push(devMenu.renderInto(this.wrapper));var css3support=QStage.supportsTransitions();this.wrapper.style[css3support.prefixedProperty]="top 0.2s ease-in";
...
label){var seperator=wrapper.createChild("div",{class:"seperator"});seperator.createChild("div",{class:"seperator-line"});seperator.createChild("div",{class:"seperator-label"},label)},renderIconButton:function(wrapper,text,iconName,closeMenuOnTap,callback){var that=this;var btnWrapper=wrapper.createChild("div",{class:"icon-item no-select"});var iconWrapper=btnWrapper.createChild("div",{class:"icon"});btnWrapper.createChild("div",{class:"label"},text);this.render24pxSvgIcon(iconWrapper,iconName);btnWrapper.addEventListener("click",
function(e){if(typeof callback=="function")callback();if(closeMenuOnTap)that.selectMenuItem(null)},false)},render24pxSvgIcon:function(wrapper,iconName){var icon=wrapper.createChildNS("svg",{class:"icon-svg-24px",width:24,height:24});var g=icon.createChildNS("g");switch(iconName){case "ICON24_HOME":g.createChildNS("path",{fill:"#434A4D",stroke:"none",d:"M3.263,22.096c0,0-0.021,0.537,0.503,0.537c0.652,0,6.05-0.008,6.05-0.008l0.009-4.957c0,0-0.085-0.818,0.708-0.818h2.51c0.938,0,0.88,0.818,0.88,0.818l-0.011,4.941c0,0,5.118,0,5.922,0c0.666,0,0.635-0.668,0.635-0.668v-9.139l-8.346-7.425l-8.86,7.425V22.096z"});
g.createChildNS("path",{fill:"#434A4D",stroke:"none",d:"M0,12.111c0,0,0.752,1.387,2.393,0l9.804-8.295l9.192,8.243c1.899,1.37,2.61,0,2.61,0L12.197,1.367L0,12.111z"});g.createChildNS("polygon",{fill:"#434A4D",stroke:"none",points:"21.166,3.797 18.803,3.797 18.813,6.664 21.166,8.662"});break;case "ICON24_SAVE":g.createChildNS("path",{fill:"#434A4D",stroke:"none",d:"M19.5,0H18v9.75H6V0H0v24h24V4.5L19.5,0z M21,22.5H3V12h18V22.5z"});g.createChildNS("rect",{fill:"#434A4D",stroke:"none",x:4.5,y:13.5,width:15,
height:1.5});g.createChildNS("rect",{fill:"#434A4D",stroke:"none",x:4.5,y:16.5,width:15,height:1.5});g.createChildNS("rect",{fill:"#434A4D",stroke:"none",x:4.5,y:19.5,width:15,height:1.5});g.createChildNS("rect",{fill:"#434A4D",stroke:"none",x:13.5,y:1.5,width:3,height:6.75});break;case "ICON24_PLAY":var d="";d+="M6,2L6,22L20,12z";g.createChildNS("path",{fill:"#434A4D",stroke:"none",d:d});break;case "ICON24_SHARE":var d="";d+="M19.229,16.965c-0.916,0-1.734,0.359-2.359,0.926l-8.593-4.999c0.063-0.277,0.11-0.557,0.11-0.845";
d+="s-0.049-0.566-0.11-0.844l8.497-4.951c0.646,0.6,1.504,0.975,2.455,0.975c2.002,0,3.617-1.611,3.617-3.614";d+="C22.846,1.615,21.23,0,19.229,0c-2,0-3.615,1.615-3.615,3.613c0,0.292,0.049,0.566,0.111,0.844L7.228,9.412";d+="C6.579,8.809,5.723,8.435,4.771,8.435c-2.003,0-3.617,1.614-3.617,3.613c0,2,1.614,3.615,3.617,3.615";d+="c0.95,0,1.808-0.375,2.455-0.975l8.578,5.008c-0.059,0.256-0.096,0.521-0.096,0.787c0,1.939,1.58,3.518,3.518,3.518";d+="c1.939,0,3.521-1.578,3.521-3.518C22.748,18.541,21.166,16.965,19.229,16.965z";
g.createChildNS("path",{fill:"#434A4D",stroke:"none",d:d});break;case "ICON24_NEW":var d="";d+="M20.294,5.87h-5.229V0.641c0-0.347-0.281-0.628-0.627-0.628H3.665c-0.347,0-0.628,0.282-0.628,0.628";d+="v22.731C3.037,23.718,3.318,24,3.665,24h16.629c0.348,0,0.629-0.282,0.629-0.628V6.498C20.923,6.151,20.642,5.87,20.294,5.87z";g.createChildNS("path",{fill:"#434A4D",stroke:"none",d:d});var d="";d+="M17.144,4.354h3.283c0.219,0,0.412-0.129,0.496-0.33c0.082-0.201,0.037-0.43-0.117-0.583l-3.285-3.285";d+="C17.419,0.056,17.286,0,17.142,0c-0.07,0-0.139,0.013-0.203,0.041c-0.201,0.083-0.332,0.277-0.332,0.494v3.285";
d+="C16.606,4.114,16.849,4.354,17.144,4.354z";g.createChildNS("path",{fill:"#434A4D",stroke:"none",d:d});break;case "ICON24_DOWNLOAD":var d="";d+="M19.399,10C18.7,6.6,15.7,4,12,4C9.1,4,6.6,5.6,5.4,8C2.3,8.4,0,10.9,0,14c0,3.301,2.7,6,6,6h13";d+="c2.8,0,5-2.199,5-5C24,12.4,21.9,10.2,19.399,10z M17,13l-5,5l-5-5h3V9h4v4H17z";g.createChildNS("path",{fill:"#434A4D",stroke:"none",d:d});break;case "ICON24_EXPORT":var d="";d+="M12 5 L7 11 H10 V19 H14 V11 H17 L12 5 Z";g.createChildNS("path",{fill:"#434A4D",
stroke:"none",d:d});g.createChildNS("rect",{fill:"#434A4D",stroke:"none",x:4,y:19,width:16,height:1});break}return icon},render32pxSvgIcon:function(wrapper,iconName){var icon=wrapper.createChildNS("svg",{class:"icon-svg-32px",width:32,height:32});var g=icon.createChildNS("g");switch(iconName){case "ICON32_MENU_App":var size=32;var padding=size*10/300;var path="M{0},{1} H{2} L {3},{4} H {0} Z".format(size/3+padding,padding,size*2/3,size-padding,size/3-padding);g.createChildNS("path",{fill:"#22CB20",
stroke:"#22CB20","stroke-width":padding,"stroke-linejoin":"round",d:path});g.createChildNS("path",{fill:"#06ACD4",stroke:"#06ACD4","stroke-width":padding,"stroke-linejoin":"round",d:path,transform:"translate(0,0), rotate({1},{0},{0})".format(size/2,1*90)});g.createChildNS("path",{fill:"#FFCC00",stroke:"#FFCC00","stroke-width":padding,"stroke-linejoin":"round",d:path,transform:"translate(0,0), rotate({1},{0},{0})".format(size/2,2*90)});g.createChildNS("path",{fill:"#FA5151",stroke:"#FA5151","stroke-width":padding,
"stroke-linejoin":"round",d:path,transform:"translate(0,0), rotate({1},{0},{0})".format(size/2,3*90)});break;case "ICON32_MENU_Option":var d="";d+="M17.8,31.7h-3.4c-0.5,0-0.7,0-2.4-4.3l-1.3-0.5c-3.6,1.6-4.1,1.6-4.2,1.6H6.2l-0.2-0.2L3.5,26 ";d+="c-0.4-0.4-0.5-0.5,1.4-4.7L4.4,20C0,18.4,0,18.2,0,17.7v-3.3c0-0.5,0-0.7,4.4-2.4l0.5-1.3c-2-4.2-1.8-4.3-1.4-4.6L6,3.6l0.3,0 ";d+="c0.4,0,1.8,0.5,4.3,1.6l1.3-0.5c1.6-4.3,1.8-4.3,2.3-4.3h3.4c0.5,0,0.7,0,2.4,4.3l1.3,0.5c3.6-1.6,4.1-1.6,4.2-1.6h0.3l0.2,0.2 ";d+=
"L28.4,6c0.4,0.4,0.5,0.5-1.4,4.7l0.5,1.3c4.4,1.6,4.4,1.7,4.4,2.3v3.3c0,0.5,0,0.7-4.4,2.4l-0.5,1.3c2,4.1,1.8,4.3,1.5,4.6 ";d+="L26,28.4l-0.3,0c-0.4,0-1.8-0.5-4.3-1.6l-1.3,0.5C18.4,31.7,18.3,31.7,17.8,31.7z M14.7,30.3h2.7c0.3-0.6,1-2.3,1.5-3.7 ";d+="l0.1-0.3l2.4-1l0.3,0.1c1.4,0.6,3.1,1.3,3.8,1.5l1.9-1.8c-0.2-0.7-1-2.3-1.6-3.6l-0.1-0.3l1-2.4l0.3-0.1 ";d+="c1.5-0.6,3.1-1.3,3.8-1.6v-2.6c-0.6-0.3-2.3-0.9-3.8-1.5l-0.3-0.1l-1-2.4l0.1-0.3c0.6-1.4,1.3-3,1.5-3.7l-1.9-1.8 ";d+="c-0.6,0.2-2.3,0.9-3.7,1.6l-0.3,0.1l-2.4-1l-0.1-0.3c-0.6-1.4-1.3-3.1-1.6-3.7h-2.7c-0.3,0.6-1,2.3-1.5,3.7L13,5.6l-2.4,1 ";
d+="l-0.3-0.1C8.9,5.9,7.2,5.2,6.5,5L4.7,6.8c0.2,0.7,1,2.3,1.6,3.6l0.1,0.3l-1,2.4l-0.3,0.1c-1.5,0.6-3.1,1.2-3.8,1.6v2.6 ";d+="c0.6,0.3,2.3,0.9,3.8,1.5l0.3,0.1l1,2.4l-0.1,0.3c-0.6,1.4-1.3,3-1.5,3.7l1.9,1.8c0.6-0.2,2.3-0.9,3.7-1.6l0.3-0.1l2.4,1 ";d+="l0.1,0.3C13.7,28.1,14.4,29.7,14.7,30.3z M16,21.5c-3.1,0-5.6-2.5-5.6-5.5c0-3,2.5-5.5,5.6-5.5c3.1,0,5.6,2.5,5.6,5.5 ";d+="C21.6,19,19.1,21.5,16,21.5z M16,11.9c-2.3,0-4.2,1.8-4.2,4.1c0,2.3,1.9,4.1,4.2,4.1c2.3,0,4.2-1.9,4.2-4.1 ";d+="C20.2,13.7,18.3,11.9,16,11.9z";
g.createChildNS("path",{fill:"#FFFFFF",stroke:"#FFFFFF","stroke-width":1,d:d});break;case "ICON32_MENU_Property":g.createChildNS("circle",{fill:"none",stroke:"#FFFFFF","stroke-width":2,cx:16,cy:16,r:15});g.createChildNS("circle",{fill:"#FFFFFF",stroke:"none",cx:8,cy:16,r:2});g.createChildNS("circle",{fill:"#FFFFFF",stroke:"none",cx:16,cy:16,r:2});g.createChildNS("circle",{fill:"#FFFFFF",stroke:"none",cx:24,cy:16,r:2});break;case "ICON32_TOOL_Picker":g.createChildNS("polygon",{fill:"none",stroke:"#FFFFFF",
"stroke-width":2,points:"8.5,2 25,17.9 16.8,17.9 22,30.2 19.5,31.2 14.3,19 8.5,24.7"});break;case "ICON32_TOOL_Move":var d="";d+="M31.917,15.593l-0.229-0.346l-4.267-4.267c-0.416-0.416-1.094-0.416-1.508,0";d+="c-0.416,0.416-0.416,1.092,0,1.508l2.444,2.445H17.066V3.642l2.445,2.445c0.414,0.416,1.092,0.416,1.508,0";d+="c0.414-0.416,0.414-1.092,0-1.508l-4.267-4.267l-0.011-0.006l-0.335-0.222L16,0h-0.004l-0.403,0.083l-0.335,0.222l-0.013,0.006";d+="l-4.267,4.267c-0.416,0.416-0.416,1.092,0,1.508s1.092,0.416,1.508,0l2.447-2.445v11.292H3.642l2.445-2.445";
d+="c0.416-0.416,0.416-1.092,0-1.508s-1.092-0.416-1.508,0l-4.267,4.267l-0.006,0.011l-0.222,0.335L0,16v0.004l0.083,0.403";d+="l0.222,0.333l0.006,0.013l4.267,4.266c0.416,0.416,1.092,0.416,1.508,0c0.416-0.414,0.416-1.092,0-1.508l-2.445-2.445h11.292";d+="v11.293l-2.445-2.445c-0.416-0.414-1.092-0.414-1.508,0c-0.416,0.416-0.416,1.094,0,1.508l4.267,4.268l0.346,0.23L15.996,32H16";d+="l0.407-0.082l0.346-0.229l4.267-4.268c0.414-0.416,0.414-1.094,0-1.508c-0.416-0.416-1.094-0.416-1.508,0l-2.445,2.445V17.066";
d+="h11.292l-2.444,2.445c-0.414,0.414-0.414,1.092,0,1.508c0.416,0.414,1.094,0.414,1.508,0l4.267-4.266l0.23-0.346L32,16.004V16";d+="L31.917,15.593z";g.createChildNS("path",{fill:"#ffffff",d:d});break;case "ICON32_TOOL_Crop":g.createChildNS("path",{fill:"none",stroke:"#FFFFFF","stroke-width":2,d:"M2,8 V2 H8 M24,2 H30 V8 M30,24 V30 H24 M8,30 H2 V24 M16,12 V20 M12,16 H 20"});break;case "ICON32_TOOL_Rectangle":g.createChildNS("path",{fill:"#ffffff",d:"M0,0v32h32V0H0z M30,30H2L2,2l28,0V30z"});break;case "ICON32_TOOL_Ellipse":g.createChildNS("path",
{fill:"#ffffff",d:"M16,0C7.2,0,0,7.2,0,16s7.2,16,16,16c8.8,0,16-7.2,16-16S24.8,0,16,0z M16,30C8.2,30,2,23.8,2,16 S8.2,2,16,2c7.7,0,14,6.3,14,14S23.7,30,16,30z"});break;case "ICON32_TOOL_Line":g.createChildNS("rect",{x:0,y:15,width:32,height:2,fill:"#ffffff"});break;case "ICON32_TOOL_Polygon":var d="";var vertexNumber=5;var x0=16;var y0=16;var r=15;var unitAngle=2*Math.PI/vertexNumber;var baseAngle=-90/180*Math.PI;for(var i=0;i<vertexNumber;i++){var radian=unitAngle*i+baseAngle;var x=x0+r*Math.cos(radian);
var y=y0+r*Math.sin(radian);var cmd=i==0?"M{0},{1}":" L{0},{1}";d+=cmd.format(x.toFixed(2),y.toFixed(2))}d+=" Z";g.createChildNS("path",{fill:"none",stroke:"#FFFFFF","stroke-width":2,d:d,"stroke-linecap":"butt","stroke-linejoin":"miter"});break;case "ICON32_TOOL_Pencil":g.createChildNS("path",{fill:"#ffffff",d:"M9.3,1.4L9.3,1.4L0,0l1.3,9.2l0,0l0,0l0,0l0,0L24.1,32l1.5-1.5l6.4-6.4L9.3,1.4z M2.4,2.5l4.4,1.2 L3.7,6.9L2.4,2.5z M4.5,9.5l4.8-4.8l14,14l-4.8,4.8L4.5,9.5z M23.9,28.8l-3.5-3.5l4.8-4.8l3.5,3.5L23.9,28.8z"});
break;case "ICON32_TOOL_Pen":var d="";d+="M26.477,15.592l-3.056-8.343c-0.13-0.351-0.531-0.639-0.894-0.639H10.5";d+="c-0.363,0-0.765,0.288-0.894,0.639L6.55,15.592c-0.129,0.351-0.095,0.91,0.075,1.242l6.887,13.971";d+="c0.17,0.332,0.606,0.603,0.968,0.603h0.896c0.363,0,0.649-0.307,0.637-0.681l-0.208-13.733";d+="c-0.013-0.376-0.264-0.862-0.559-1.082c0,0-1.201-0.894-1.201-2.041c0-1.411,1.105-2.555,2.469-2.555";d+="c1.364,0,2.468,1.144,2.468,2.555c0,1.147-1.2,2.041-1.2,2.041c-0.295,0.22-0.547,0.706-0.561,1.082l-0.227,13.749";
d+="c-0.013,0.376,0.272,0.684,0.635,0.684h0.926c0.363,0,0.798-0.271,0.969-0.604l6.879-13.989";d+="C26.572,16.502,26.605,15.943,26.477,15.592z";g.createChildNS("path",{fill:"none",stroke:"#FFFFFF","stroke-width":2,d:d});d="";d+="M9.397,5.292h14.231c0.361,0,0.66-0.307,0.66-0.682V1.257c0-0.375-0.299-0.683-0.66-0.683";d+="H9.397c-0.363,0-0.66,0.307-0.66,0.683V4.61C8.738,4.985,9.034,5.292,9.397,5.292z";g.createChildNS("path",{fill:"none",stroke:"#FFFFFF","stroke-width":2,d:d});break;case "ICON32_TOOL_Label":g.createChildNS("polygon",
{style:"fill:#FFFFFF;",points:"23.91,10.705 23.91,12.348 30.359,12.348 30.359,21.393 23.91,21.393 23.91,23.033 32,23.033 32,10.705"});g.createChildNS("polygon",{style:"fill:#FFFFFF;",points:"17.599,21.393 1.641,21.393 1.641,12.348 17.599,12.348 17.599,10.705 0,10.705 0,23.033 17.599,23.033"});var d="";d+="M21.942,26.466c-0.067-0.069-0.114-0.147-0.153-0.249V7.854l-0.005-0.125";d+="c0.032-0.16,0.098-0.292,0.204-0.403C22.302,7,22.912,6.93,23.369,6.93h0.854V4.794";d+="h-0.854c-1.096,0-1.979,0.264-2.631,0.784c-0.655-0.556-1.561-0.837-2.689-0.837";
d+="h-0.855v2.138h0.855c0.454,0,1.064,0.067,1.379,0.396c0.068,0.069,0.121,0.154,0.155,0.251";d+="v18.403c0,0.012-0.002,0.31-0.223,0.537c-0.316,0.329-0.927,0.4-1.384,0.4h-0.854V29h0.854";d+="c1.121,0,2.02-0.276,2.674-0.819C21.306,28.728,22.202,29,23.32,29h0.857v-2.134H23.32";d+="C22.864,26.864,22.256,26.795,21.942,26.466z";g.createChildNS("path",{fill:"#FFFFFF",stroke:"none",d:d});break;case "ICON32_TOOL_Map":var d="";d+="M10.866,17.107c1.119,0,2.025,0.906,2.025,2.025c0,1.117-0.907,2.025-2.025,2.025S8.84,20.25,8.84,19.133";
d+="C8.84,18.014,9.747,17.107,10.866,17.107z";g.createChildNS("path",{fill:"#FFFFFF",stroke:"none",d:d});d="";d+="M30.59,18.734l1.373-10.828l-9.852-3.738L10.295,9.813L0,5.465l1.39,11.551L0.058,28.389l10.938,2.053";d+="l11.158-2.125L32,30.588L30.59,18.734z M22.074,5.315l8.727,3.288l-0.602,4.734l-8.695-2.2L22.074,5.315z M20.92,5.917";d+="l-0.459,4.957l-0.008-0.002l-6.022,0.777l-0.036,0.133c-0.008,0.029-0.706,2.61-1.442,3.784c-0.477-0.28-1.012-0.468-1.585-0.538";d+="l-0.488-4.316L20.92,5.917z M20,16.973l-5.233,0.789c-0.282-0.797-0.799-1.481-1.471-1.971c0.685-1.094,1.304-3.211,1.459-3.77";
d+="l5.667-0.731L20,16.973z M9.815,10.764l0.484,4.274c-1.591,0.219-2.896,1.342-3.374,2.835l-4.462-0.691l-0.429-4.139";d+="c1.479-0.098,2.589-0.579,3.3-1.438c0.641-0.773,0.826-1.688,0.861-2.37L9.815,10.764z M1.288,7.163l4.499,1.9";d+="c-0.013,0.627-0.155,1.538-0.769,2.279c-0.641,0.773-1.661,1.206-3.036,1.292L1.288,7.163z M1.23,27.527l1.107-9.289l4.401,0.682";d+="c-0.003,0.07-0.01,0.141-0.01,0.213c0,0.967,0.336,1.857,0.896,2.563l-2.286,1.836l-0.72,4.631L1.23,27.527z M9.853,29.145";d+="l-4.832-0.906l0.698-4.488l2.175-1.746c0.667,0.691,1.57,1.15,2.579,1.246L9.853,29.145z M7.549,19.133";
d+="c0-1.83,1.488-3.317,3.316-3.317c1.83,0,3.317,1.487,3.317,3.317c0,1.828-1.488,3.316-3.317,3.316";d+="C9.037,22.449,7.549,20.961,7.549,19.133z M10.995,29.359l-0.093-0.018l0.646-6.133c1.958-0.326,3.457-2.027,3.457-4.076";d+="c0-0.111-0.008-0.221-0.017-0.328l5.116-0.771l0.428,4.363c-2.244,0.104-3.922,0.795-4.979,2.068";d+="c-1.231,1.488-1.289,3.342-1.226,4.26L10.995,29.359z M14.737,28.646c-0.05-0.871,0.022-2.578,1.132-3.918";d+="c0.982-1.186,2.567-1.826,4.702-1.924l0.455,4.643L14.737,28.646z M21.465,11.551l8.684,2.196l-0.52,4.103l-8.574-1.035";
d+="L21.465,11.551z M28.447,21.393l-3.951-0.469l0.725,7.008l-3.166-0.68l-0.898-9.377l0.092-0.014l8.289,1.051l1.227,10.301";d+="l-5.129-1.184l-0.678-6.637l3.521,0.408L28.447,21.393z";g.createChildNS("path",{fill:"#FFFFFF",stroke:"none",d:d});break;case "ICON32_TOOL_Poi":var d="";d+="M16.791,0c-5.988,0-10.86,4.872-10.86,10.86c0,2.406,1.417,6.065,4.459,11.517";d+="c2.076,3.72,4.142,6.861,4.229,6.994l1.493,2.264C16.262,31.862,16.518,32,16.791,32s0.528-0.138,0.679-0.365l1.494-2.264";d+="c0.086-0.131,2.137-3.245,4.229-6.994c3.042-5.45,4.459-9.11,4.459-11.517C27.651,4.872,22.779,0,16.791,0z M21.771,21.584";
d+="c-2.063,3.696-4.081,6.762-4.166,6.891l-0.814,1.234l-0.814-1.234c-0.085-0.13-2.12-3.223-4.167-6.891";d+="C8.989,16.528,7.558,12.92,7.558,10.86c0-5.091,4.142-9.233,9.233-9.233s9.232,4.142,9.232,9.233";d+="C26.023,12.92,24.593,16.528,21.771,21.584z";g.createChildNS("path",{fill:"#FFFFFF",stroke:"none",d:d});d="";d+="M16.791,4.771c-3.32,0-6.021,2.701-6.021,6.021c0,3.32,2.701,6.021,6.021,6.021s6.021-2.701,6.021-6.021";d+="C22.813,7.472,20.111,4.771,16.791,4.771z M16.791,15.187c-2.423,0-4.394-1.971-4.394-4.394s1.971-4.394,4.394-4.394";
d+="s4.394,1.971,4.394,4.394S19.214,15.187,16.791,15.187z";g.createChildNS("path",{fill:"#FFFFFF",stroke:"none",d:d});break;case "ICON32_TOOL_Block":var d="";d+="M28.405,4.527h-9.918C18.218,4.527,18,4.746,18,5.015v1.46c0,0.269,0.218,0.487,0.487,0.487h9.918";d+="c0.641,0,1.161,0.521,1.161,1.162v16.095c0,0.641-0.521,1.16-1.161,1.16h-9.918c-0.27,0-0.487,0.219-0.487,0.488v1.457";d+="c0,0.27,0.218,0.49,0.487,0.49h9.918c1.982,0,3.595-1.615,3.595-3.596V8.124C32,6.141,30.387,4.527,28.405,4.527z";g.createChildNS("path",
{fill:"#FFFFFF",stroke:"none",d:d});d="";d+="M3.595,27.813h9.918c0.27,0,0.487-0.218,0.487-0.487v-1.461c0-0.268-0.218-0.486-0.487-0.486H3.595";d+="c-0.641,0-1.161-0.521-1.161-1.162V8.123c0-0.642,0.521-1.16,1.161-1.16h9.918c0.27,0,0.487-0.219,0.487-0.489V5.016";d+="c0-0.27-0.218-0.489-0.487-0.489H3.595C1.612,4.527,0,6.141,0,8.123v16.094C0,26.2,1.613,27.813,3.595,27.813z";g.createChildNS("path",{fill:"#FFFFFF",stroke:"none",d:d});g.createChildNS("path",{fill:"#FFFFFF",stroke:"none",d:"M15,9h2v14h-2V9z"});
break;case "ICON32_TOOL_Eraser":var d="";d+="M28.405,4.527h-9.918C18.218,4.527,18,4.746,18,5.015v1.46c0,0.269,0.218,0.487,0.487,0.487h9.918";d+="c0.641,0,1.161,0.521,1.161,1.162v16.095c0,0.641-0.521,1.16-1.161,1.16h-9.918c-0.27,0-0.487,0.219-0.487,0.488v1.457";d+="c0,0.27,0.218,0.49,0.487,0.49h9.918c1.982,0,3.595-1.615,3.595-3.596V8.124C32,6.141,30.387,4.527,28.405,4.527z";g.createChildNS("path",{fill:"#FFFFFF",stroke:"none",d:d});d="";d+="M3.595,27.813h9.918c0.27,0,0.487-0.218,0.487-0.487v-1.461c0-0.268-0.218-0.486-0.487-0.486H3.595";
d+="c-0.641,0-1.161-0.521-1.161-1.162V8.123c0-0.642,0.521-1.16,1.161-1.16h9.918c0.27,0,0.487-0.219,0.487-0.489V5.016";d+="c0-0.27-0.218-0.489-0.487-0.489H3.595C1.612,4.527,0,6.141,0,8.123v16.094C0,26.2,1.613,27.813,3.595,27.813z";g.createChildNS("path",{fill:"#FFFFFF",stroke:"none",d:d});break;default:var d="";d+="M31.732,6.211l0.023-0.058L16.37,0L9.131,2.797l-0.046-0.02L9.064,2.824l-8.78,3.393L0.232,6.197v0.04";d+="L0.23,6.237l0.002,0.006v19.735l15.205,5.998V32h1.127v-0.023l15.205-5.998V6.197L31.732,6.211z M30.643,16.468l-6.395,2.346";
d+="v-8.439l6.395-2.523V16.468z M15.438,13.405v8.231l-6.513-2.355v-8.446L15.438,13.405z M9.68,9.924l6.545-2.722l6.093,2.723";d+="L16,12.417L9.68,9.924z M16.564,13.405l6.557-2.586v8.408l-6.557,2.404V13.405L16.564,13.405z M23.785,9.346l-6.129-2.739";d+="l5.984-2.49l6.66,2.659L23.785,9.346z M16.356,1.21l5.788,2.31l-5.902,2.455l-5.661-2.529L16.356,1.21z M9.103,4.019l5.709,2.551";d+="L8.175,9.329L1.84,6.83L9.103,4.019z M7.799,10.392v8.481l-6.439-2.329V7.852L7.799,10.392z M1.359,17.742l6.439,2.33v7.68";
d+="l-6.439-2.541V17.742z M8.924,28.195v-7.717l6.513,2.357v7.928L8.924,28.195z M16.564,22.832l6.557-2.404v7.75l-6.557,2.586V22.832";d+="L16.564,22.832z M24.248,27.734v-7.721l6.395-2.346v7.543L24.248,27.734z";g.createChildNS("path",{fill:"#FFFFFF",stroke:"none",d:d});break}return icon}};function ToolMenu(pane){this.pane=pane;this.width=236;this.title="Tools";this.button=null;this.options=[]}
...
function Painting(workspace){this.workspace=workspace;this.shapes=[];this.maxShapeId=-1;this.name="";this.toolType=ToolTypes.Picker;this.modified=false;this.fill={color:Config.DrawingBoard.fillColor,opacity:Config.DrawingBoard.fillOpacity};this.stroke={color:Config.DrawingBoard.strokeColor,opacity:Config.DrawingBoard.strokeOpacity,width:Config.DrawingBoard.strokeWidth};this.selectingShape=null;this.selectedShape=null;this.onToolChanged=null;this.onFillChanged=null;this.onStrokeChanged=null;this.onShapeChanged=
null;this.onShapeAppended=null;this.onShapeRemoved=null;this.onShapeSorted=null;this.onProjectChanged=null}
Painting.prototype={hasEllipse:function(){for(var i=0;i<this.shapes.length;i++)if(this.shapes[i].getType()==ShapeTypes.Ellipse)return true;return false},removeAllShapes:function(){while(this.shapes.length>0)this.shapes[0].remove()},reset:function(){this.selectTool(ToolTypes.Picker);this.setFill(Config.DrawingBoard.fillColor,Config.DrawingBoard.fillOpacity);this.setStroke(Config.DrawingBoard.strokeColor,Config.DrawingBoard.strokeOpacity,Config.DrawingBoard.strokeWidth);this.removeAllShapes()},getNextId:function(){return++this.maxShapeId},
createShape:function(type,container,transformInfo){var shape=null;switch(type){case ShapeTypes.Line:shape=new Line(container,transformInfo,null,null);break;case ShapeTypes.Rectangle:shape=new Rectangle(container,transformInfo,null,null);break;case ShapeTypes.Ellipse:shape=new Ellipse(container,transformInfo,null,null);break;case ShapeTypes.Polygon:shape=new Polygon(container,transformInfo,null,null);break;case ShapeTypes.Freeline:shape=new Freeline(container,transformInfo,null,null);break;case ShapeTypes.Label:shape=
new Label(container,transformInfo,null,null);break;case ShapeTypes.Polyline:shape=new Polyline(container,transformInfo,null,null);break}this.appendShape(shape);this.selectShape(shape);return shape},appendShape:function(shape){var that=this;this.shapes.push(shape);shape.onStatusChanged=function(){that.projectChanged()};if(typeof this.onShapeAppended=="function")this.onShapeAppended(shape);this.projectChanged()},searchShape:function(shape){for(var i=0;i<this.shapes.length;i++)if(this.shapes[i]==shape)return i;
return-1},changeShapePosition:function(shape,newIndex){if(newIndex>this.shapes.length-1||newIndex<0)return;var oldIndex=this.searchShape(shape);if(oldIndex<0)return;if(oldIndex==newIndex)return;if(oldIndex<newIndex)for(var i=oldIndex;i<newIndex;i++)this.shapes[i]=this.shapes[i+1];else for(var i=oldIndex;i>newIndex;i--)this.shapes[i]=this.shapes[i-1];this.shapes[newIndex]=shape;if(typeof this.onShapeSorted=="function")this.onShapeSorted(shape,oldIndex,newIndex);this.projectChanged()},removeShape:function(shape){var index=
this.searchShape(shape);if(index>=0)this.shapes.splice(index,1);if(typeof this.onShapeRemoved=="function")this.onShapeRemoved(shape,index);this.projectChanged()},selectTool:function(newToolType){if(this.toolType==newToolType)return;var oldToolType=this.toolType;this.toolType=newToolType;if(typeof this.onToolChanged=="function")this.onToolChanged(oldToolType,newToolType)},setFill:function(color,opacity){this.fill.color=color;this.fill.opacity=opacity;if(typeof this.onFillChanged=="function")this.onFillChanged(color,
opacity)},setStroke:function(color,opacity,width){this.stroke.color=color;this.stroke.opacity=opacity;this.stroke.width=width;if(typeof this.onStrokeChanged=="function")this.onStrokeChanged(color,opacity,width)},selectShape:function(shape){if(this.selectedShape==shape)return;else{if(this.selectedShape!=null)this.selectedShape.setSelected(false);this.selectedShape=shape;if(shape!=null)shape.setSelected(true);if(typeof this.onShapeChanged=="function")this.onShapeChanged(shape)}},selectShapeById:function(id){if(id<
0)this.selectShape(null);for(var i=0;i<this.shapes.length;i++)if(this.shapes[i].id==id)this.selectShape(this.shapes[i])},isShapeTool:function(){return this.toolType==ToolTypes.Rectangle||this.toolType==ToolTypes.Ellipse||this.toolType==ToolTypes.Line||this.toolType==ToolTypes.Polygon||this.toolType==ToolTypes.Pencil||this.toolType==ToolTypes.Pen||this.toolType==ToolTypes.Label},projectChanged:function(){this.modified=true;if(typeof this.onProjectChanged=="function")this.onProjectChanged()},getCanvas:function(){var frame=
this.workspace.drawingBoard.canvasFrame.transFrame;return{position:frame.getPosition(),boundary:frame.getBoundary()}},setCanvas:function(jsonObject){var frame=this.workspace.drawingBoard.canvasFrame.transFrame;var position=jsonObject.position;var boundary=jsonObject.boundary;frame.moveTo(position.x,position.y,false);frame.transformTo(boundary.left,boundary.top,boundary.right,boundary.bottom,true)},serialize:function(type){var o={name:this.name,canvas:this.getCanvas(),shapes:[]};for(var i=0;i<this.shapes.length;i++)o.shapes.push(this.shapes[i].toJsonObject());
if(type=="JSON")return o;else if(type=="STRING")return JSON.stringify(o);else return null},deserialize:function(obj,type){this.workspace.autoSaveEnabled=false;this.removeAllShapes();var o=null;if(type=="JSON")o=obj;else if(type=="STRING")o=JSON.stringify(o);else{this.workspace.autoSaveEnabled=true;return false}this.setCanvas(o.canvas);this.name=o.name||"";for(var i=0;i<o.shapes.length;i++){var shape=BaseShape.fromJsonObject(o.shapes[i]);this.appendShape(shape)}this.workspace.autoSaveEnabled=true;
return true},toSVG:function(){var rect=this.workspace.getExportRect();var output="";output+='<?xml version="1.0" encoding="utf-8"?>'+"\n";output+='<!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 1.1//EN" "http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd">'+"\n";output+='<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="{0}" height="{1}" style="border: 1px solid #000; overflow: hidden">'.format(rect.width,rect.height)+"\n";output+="<g>"+"\n";for(var i=0;i<
this.shapes.length;i++)output+=this.shapes[i].toString(SerializeTypes.SVG)+"\n";output+="</g>"+"\n";output+="</svg>"+"\n";return output},toCanvas:function(){var rect=this.workspace.getExportRect();var output="";output+="<!DOCTYPE HTML>"+"\n";output+="<html>"+"\n";output+="<head>"+"\n";output+="<title>Canvas Output</title>"+"\n";if(this.hasEllipse()){output+='<script type="text/javascript">'+"\n";output+="var canvasPrototype = window.CanvasRenderingContext2D && CanvasRenderingContext2D.prototype;"+
"\n";output+="canvasPrototype.bezierEllipse = function (x, y, a, b) {"+"\n";output+="    var ox = 0.5 * a,"+"\n";output+="    oy = 0.6 * b;"+"\n";output+="    this.save();"+"\n";output+="    this.translate(x, y);"+"\n";output+="    this.beginPath();"+"\n";output+="    this.moveTo(0, b);"+"\n";output+="    this.bezierCurveTo(ox, b, a, oy, a, 0);"+"\n";output+="    this.bezierCurveTo(a, -oy, ox, -b, 0, -b);"+"\n";output+="    this.bezierCurveTo(-ox, -b, -a, -oy, -a, 0);"+"\n";output+="    this.bezierCurveTo(-a, oy, -ox, b, 0, b);"+
"\n";output+="    this.closePath();"+"\n";output+="    this.restore();"+"\n";output+="}"+"\n";output+="\x3c/script>"+"\n"}output+="</head>"+"\n";output+='<body style="background-color: #666;">'+"\n";output+='<canvas id="paint-canvas" width="{0}" height="{1}" style="border: 1px solid #000; background-color: {2}"></canvas>'.format(rect.width,rect.height,Config.General.drawingBackground)+"\n";output+='<script type="text/javascript">'+"\n";output+='var c=document.getElementById("paint-canvas");'+"\n";
output+='var cxt=c.getContext("2d");'+"\n";for(var i=0;i<this.shapes.length;i++)output+=this.shapes[i].toString(SerializeTypes.Canvas)+"\n";output+="\x3c/script>"+"\n";output+="</body>"+"\n";output+="</html>"+"\n";return output}};
...
DrawingBoard.prototype={renderIntoWorkspace:function(){var that=this;this.wrapper=this.workspace.getWrapper().createChild("div",{id:"drawing-board"});this.svg=this.wrapper.createChildNS("svg",{id:"drawing-svg",width:this.svgSize.width,height:this.svgSize.height,style:"background-color:"+Config.General.canvasBackground});this.underGroup=this.svg.createChildNS("g",{"data-label":"Group of Frame"});this.shapeGroup=this.svg.createChildNS("g",{"data-label":"Group of Shapes"});this.upperGroup=this.svg.createChildNS("g",
{"data-label":"Group of Misc"});this.canvasFrame=new CanvasFrame(this.underGroup);var svgEvent=workspace.getEventManager(this.wrapper);svgEvent.addListener("capture",function(e,args){that.processCapture(e,args)});svgEvent.addListener("release",function(e,args){that.processRelease(e,args)});svgEvent.addListener("drag",function(e,args){that.processDrag(e,args)})},processCapture:function(e,args){var a=document.elementFromPoint(e.pageX,e.pageY);var selectingShape=workspace.painting.selectingShape;var selectedShape=
workspace.painting.selectedShape;if(workspace.painting.toolType==ToolTypes.Picker)if(selectingShape!=null){if(selectingShape!=selectedShape)workspace.painting.selectShape(selectingShape);workspace.painting.selectingShape=null}else workspace.painting.selectShape(null);if(workspace.painting.toolType==ToolTypes.Pen){var point=this.transformPoint({x:args.current.x,y:args.current.y});if(selectingShape!=null&&selectedShape!=selectingShape){workspace.painting.selectShape(selectingShape);workspace.painting.selectingShape=
null;selectingShape.switchPathEditing(true)}else if(selectedShape!=null&&selectedShape.isPathShape()){var selectingPoint=selectedShape.selectingPoint;var selectedPoint=selectedShape.selectedPoint;if(selectingPoint!=null){if(selectingPoint.selected)if(selectingPoint.selectingCtrl!=null)selectingPoint.selectingCtrl=null;else selectedShape.selectPoint(null);else selectedShape.selectPoint(selectingPoint);selectedShape.selectingPoint=null}else{var dx=point.x-selectedShape.getOffset().x;var dy=point.y-
selectedShape.getOffset().y;var orgPoint=selectedShape.transFrame.getOriginalPoint(dx,dy);var newAnchor=null;if(selectedShape.selectedPoint==null);else if(selectedShape.selectedPoint.isLast()||selectedShape.anchorPoints.length<2)newAnchor=selectedShape.addAndRenderPoint(orgPoint.x,orgPoint.y,true);else newAnchor=selectedShape.insertAndRenderPoint(orgPoint.x,orgPoint.y,selectedShape.selectedPoint);selectedShape.selectPoint(newAnchor)}}else{var transformInfo={position:{x:point.x,y:point.y},boundary:{left:0,
top:0,right:0,bottom:0},rotation:{cx:0,cy:0,angle:0}};var path=workspace.painting.createShape(ShapeTypes.Polyline,this.shapeGroup,transformInfo);path.selectLastPoint()}}if(typeof this.onCaptured=="function")this.onCaptured()},processRelease:function(e,args){if(workspace.painting.toolType==ToolTypes.Picker);else if(workspace.painting.toolType==ToolTypes.Pen){var selectedShape=workspace.painting.selectedShape;if(selectedShape!=null){var selectedPoint=selectedShape.selectedPoint;if(selectedPoint!=null){var ctrl=
selectedPoint.bezierLastCtrl;if(ctrl!=null&&ctrl.captured)ctrl.setCaptured(false)}}}else{workspace.painting.selectShape(null);if(!Config.General.lockTool&&workspace.painting.isShapeTool())workspace.painting.selectTool(ToolTypes.Picker)}this.workspace.scaleplate.moveIndicator(-1,-1)},processDrag:function(e,args){var currentPoint=this.transformPoint(args.current);var selectedShape=workspace.painting.selectedShape;if(selectedShape==null){var transformInfo={position:this.transformPoint(args.begin),boundary:{left:0,
top:0,right:0,bottom:0},rotation:{cx:0,cy:0,angle:0}};switch(workspace.painting.toolType){case ToolTypes.Picker:break;case ToolTypes.Move:this.moveBoard(args.previous.dx,args.previous.dy);break;case ToolTypes.Line:workspace.painting.createShape(ShapeTypes.Line,this.shapeGroup,transformInfo);break;case ToolTypes.Rectangle:workspace.painting.createShape(ShapeTypes.Rectangle,this.shapeGroup,transformInfo);break;case ToolTypes.Ellipse:workspace.painting.createShape(ShapeTypes.Ellipse,this.shapeGroup,
...
"",true);this.objectListDoms.push(dom);if(this.isIdSelected(shape.id))dom.addClassName("selected");var titleDom=dom.createChild("div",{class:"item-title"},shape.toString(SerializeTypes.Abstract),"",false);titleDom.addEventListener("click",function(e){var id=this.parentNode.getAttribute("data-id");that.workspace.selectShapeById(id)})},updateList:function(){while(this.objectListDoms.length>0){var dom=this.objectListDoms.pop();this.objectListContainer.removeChild(dom)}var shapes=this.workspace.painting.shapes;
for(var i=0;i<shapes.length;i++){var shape=shapes[i];this.insertItem(shape)}},updateSummary:function(){var shapes=this.workspace.painting.shapes;this.summaryLabel.textContent="Objects Number: "+shapes.length},isIdSelected:function(id){for(var i=0;i<this.selectedShapes.length;i++)if(this.selectedShapes[i].id==id)return true;return false},appendShape:function(shape){if(!this.showing)return;this.insertItem(shape);this.updateSummary()},removeShape:function(shape,index){if(!this.showing)return;this.objectListContainer.removeChild(this.objectListDoms[index]);
...
PromptDialog.prototype={renderIntoWorkspace:function(){var that=this;this.wrapper=this.workspace.getWrapper().createChild("div",{id:"prompt-wrapper",style:"display:none"});this.dlgBox=this.wrapper.createChild("div",{class:"prompt-dlg"});this.textBox=this.dlgBox.createChild("input",{id:"prompt-dlg__text-box",type:"text",class:"text"});this.button=this.dlgBox.createChild("input",{type:"button",class:"button",value:"OK"});this.button.addEventListener("click",function(){that.setShowing(false)},false)},
selectAll:function(){this.textBox.focus();this.textBox.select()},setShowing:function(show){this.wrapper.updateStyle("display",show?null:"none")},setText:function(text){this.textBox.value=text},setEnabled:function(enabled){if(enabled){this.textBox.removeAttribute("readonly");this.button.removeAttribute("disabled")}else{this.textBox.setAttribute("readonly","readonly");this.button.setAttribute("disabled","disabled")}},resize:function(width,height){this.wrapper.updateStyle("width",width+"px");this.wrapper.updateStyle("height",
height+"px");this.dlgBox.updateStyle("margin-top",height/2-50+"px")}};function TextPanel(workspace){this.workspace=workspace;this.wrapper=null;this.contetArea=null;this.buttonArea=null;this.titleBar=null;this.textArea=null}
TextPanel.prototype={renderIntoWorkspace:function(){var that=this;this.wrapper=this.workspace.getWrapper().createChild("div",{id:"text-panel",style:"display: none; left: 0px; top: 0px; width: 0px; height: 0px"});this.contetArea=this.wrapper.createChild("div",{class:"content-area"});this.titleBar=this.contetArea.createChild("div",{class:"title-bar"});var buttons=this.titleBar.createChild("div",{class:"buttons"});this.buttonArea=buttons.createChild("div",{style:"display: inline"});var btnClose=buttons.createChild("div",
{class:"button",title:"close"});btnClose.addEventListener("click",function(e){that.hide()});var svg=btnClose.createChildNS("svg",{class:"top-menu-icon",width:16,height:16});var g=svg.createChildNS("g");g.createChildNS("path",{d:"M0 0 L{0} {0} M{0} 0 L0 {0}".format(16),stroke:"#FE5722","stroke-width":2,fill:"none"});this.textArea=this.contetArea.createChild("textarea",{id:"text-panel__text-area",class:"text-area"})},clearButtons:function(){while(this.buttonArea.hasChildNodes())this.buttonArea.removeChild(this.buttonArea.firstChild)},
...
this.svgGroup.parentNode.firstChild);workspace.painting.changeShapePosition(this,0)},toString:function(serializeType){var typeName="Unknown";switch(this.getType()){case ShapeTypes.Line:typeName="Line";break;case ShapeTypes.Rectangle:typeName="Rectangle";break;case ShapeTypes.Ellipse:typeName="Ellipse";break;case ShapeTypes.Polygon:typeName="Polygon";break;case ShapeTypes.Polyline:typeName="Polyline";break;case ShapeTypes.Freeline:typeName="Freeline";break;case ShapeTypes.Label:typeName="Label";break}switch(serializeType){case SerializeTypes.TypeOnly:return typeName;
break;case SerializeTypes.Abstract:return"{0} [#{1}]".format(typeName,this.id);break;case SerializeTypes.JSON:return JSON.stringify(this.getJsonObject());break;default:return null;break}},toJsonObject:function(){var o={id:this.id,type:this.getType(),position:this.transFrame.getPosition(),boundary:this.transFrame.getBoundary(),rotation:this.transFrame.getRotation(),points:[],properties:[]};for(var i=0;i<this.anchorPoints.length;i++){var point=this.anchorPoints[i];var controller=point.bezierFirstCtrl;
if(controller==null)o.points.push([point.x,point.y]);else o.points.push([point.x,point.y,controller.x,controller.y])}for(var i=0;i<this.propertyDict.length;i++){var property=this.propertyDict[i];o.properties.push([property.key,property.value])}return o},isPathShape:function(){return this.getType()==ShapeTypes.Polyline||this.getType()==ShapeTypes.Polygon}};var BaseShape=Class.extend(base_shape_prototype);
...
boundary.ry;var ox=.5*rx;var oy=.6*ry;var d="";d+=QStage.buildPathCommand("M",[0+cx,ry+cy]);d+=QStage.buildPathCommand("C",[ox+cx,ry+cy,rx+cx,oy+cy,rx+cx,0+cy]);d+=QStage.buildPathCommand("C",[rx+cx,-oy+cy,ox+cx,-ry+cy,0+cx,-ry+cy]);d+=QStage.buildPathCommand("C",[-ox+cx,-ry+cy,-rx+cx,-oy+cy,-rx+cx,0+cy]);d+=QStage.buildPathCommand("C",[-rx+cx,oy+cy,-ox+cx,ry+cy,0+cx,ry+cy]);d+=QStage.buildPathCommand("Z");return d},toString:function(serializeType){var baseResult=this._super(serializeType);if(baseResult!==
null)return baseResult;var position=this.transFrame.getPosition();var boundary=QStage.enhanceBoundary(this.transFrame.getBoundary(),true);var rotation=this.transFrame.getRotation();if(serializeType===SerializeTypes.SVG)return"[SVG "+this.toString(SerializeTypes.TypeOnly)+"]";if(serializeType===SerializeTypes.Canvas)return"[Canvas "+this.toString(SerializeTypes.TypeOnly)+"]";return null},render:function(){var that=this;this.shapeElement=this.svgGroup.createChildNS("path",{"data-label":"Shape",fill:QStage.brushToRgba(this.prop("fill")),
stroke:QStage.brushToRgba(this.prop("stroke")),"stroke-width":this.prop("stroke").width,d:this.getPath()});this.renderComplete()}};var Ellipse=BaseShape.extend(ellipse_prototpye);
var freeline_prototype={getType:function(){return ShapeTypes.Freeline},init:function(parent,transformInfo,pointsInArray,initialProperties){var that=this;this._super(parent,transformInfo,pointsInArray,initialProperties);this.prop("applyFill",false);if(pointsInArray==null){this.addPoint(transformInfo.boundary.left,transformInfo.boundary.top,null);this.addPoint(transformInfo.boundary.right,transformInfo.boundary.bottom,null)}this.render()},toString:function(serializeType){var baseResult=this._super(serializeType);
if(baseResult!==null)return baseResult;var position=this.transFrame.getPosition();var boundary=QStage.enhanceBoundary(this.transFrame.getBoundary(),true);var rotation=this.transFrame.getRotation();if(serializeType===SerializeTypes.SVG)return"[SVG "+this.toString(SerializeTypes.TypeOnly)+"]";if(serializeType===SerializeTypes.Canvas)return"[Canvas "+this.toString(SerializeTypes.TypeOnly)+"]";return null},render:function(){var that=this;this.shapeElement=this.svgGroup.createChildNS("path",{"data-label":"Shape",
...
optionArray.push(["spacingAndGlyphs","SpacingAndGlyphs"]);return optionArray}})},toString:function(serializeType){var baseResult=this._super(serializeType);if(baseResult!==null)return baseResult;var position=this.transFrame.getPosition();var boundary=QStage.enhanceBoundary(this.transFrame.getBoundary(),true);var rotation=this.transFrame.getRotation();if(serializeType===SerializeTypes.SVG)return"[SVG "+this.toString(SerializeTypes.TypeOnly)+"]";if(serializeType===SerializeTypes.Canvas)return"[Canvas "+
this.toString(SerializeTypes.TypeOnly)+"]";return null},render:function(){var boundary=QStage.enhanceBoundary(this.transFrame.getBoundary(),true);this.shapeElement=this.svgGroup.createChildNS("text",{"data-label":"Label",fill:QStage.brushToRgba(this.prop("fill")),class:"no-select",x:boundary.left,y:boundary.bottom,"font-size":boundary.height,textLength:boundary.width,lengthAdjust:this.prop("lengthAdjust")},this.prop("text"));this.renderComplete()}};var Label=BaseShape.extend(label_prototype);
...
that.transFrame.getBoundary();that.anchorPoints[0].movePointTo(boundary.left,boundary.top,false,false);that.anchorPoints[1].movePointTo(boundary.right,boundary.bottom,false,true)})},toString:function(serializeType){var baseResult=this._super(serializeType);if(baseResult!==null)return baseResult;var position=this.transFrame.getPosition();var boundary=QStage.enhanceBoundary(this.transFrame.getBoundary(),true);var rotation=this.transFrame.getRotation();if(serializeType===SerializeTypes.SVG)return"[SVG "+
this.toString(SerializeTypes.TypeOnly)+"]";if(serializeType===SerializeTypes.Canvas)return"[Canvas "+this.toString(SerializeTypes.TypeOnly)+"]";return null},render:function(){var that=this;this.shapeElement=this.svgGroup.createChildNS("path",{"data-label":"Shape",d:this.getPathData(false),fill:"none",stroke:QStage.brushToRgba(this.prop("stroke")),"stroke-width":this.prop("stroke").width});this.ghostElement=this.svgGroup.createChildNS("path",{"data-label":"Ghost",d:this.getPathData(false),fill:"none",
stroke:Config.Shape.ghostStroke,"stroke-width":Config.Shape.ghostWidth});var shapeEvent=workspace.getEventManager(this.ghostElement).addListener("capture",function(e,args){if(workspace.painting.toolType==ToolTypes.Picker)that.transFrame.saveStatus()}).addListener("drag",function(e,args){if(workspace.painting.toolType==ToolTypes.Picker)that.transFrame.move(args.begin.dx,args.begin.dy)});this.renderComplete();this.transFrame.setProperty("nilWidthAccepted",true);this.transFrame.setProperty("nilHeightAccepted",
...
number)this.addAndRenderPoint(0,0,false);else{var point=this.anchorPoints.pop();point.svgGroup.parentNode.removeChild(point.svgGroup);point=null;delete point}this.alignPointsToBoundary(this.transFrame.getBoundary())},toString:function(serializeType){var baseResult=this._super(serializeType);if(baseResult!==null)return baseResult;var position=this.transFrame.getPosition();var boundary=QStage.enhanceBoundary(this.transFrame.getBoundary(),true);var rotation=this.transFrame.getRotation();if(serializeType===
SerializeTypes.SVG)return"[SVG "+this.toString(SerializeTypes.TypeOnly)+"]";if(serializeType===SerializeTypes.Canvas)return"[Canvas "+this.toString(SerializeTypes.TypeOnly)+"]";return null},render:function(){var that=this;this.shapeElement=this.svgGroup.createChildNS("path",{"data-label":"Shape",d:this.getPathData(false),fill:QStage.brushToRgba(this.prop("fill")),stroke:QStage.brushToRgba(this.prop("stroke")),"stroke-width":this.prop("stroke").width});this.renderComplete()},alignPointsToBoundary:function(boundary){var enhancedNewBoundary=
QStage.enhanceBoundary(boundary,false);var points=this.getPointsOnBoundary(enhancedNewBoundary,this.anchorPoints.length);for(var i=0;i<points.length;i++){var isLast=i==points.length-1;var point=points[i];this.anchorPoints[i].movePointTo(point.x,point.y,false,isLast)}},getPointOnCircle:function(x0,y0,rx,ry,vertexNumber,pointIndex){var unitAngle=2*Math.PI/vertexNumber;var baseAngle=-90/180*Math.PI;var radian=unitAngle*pointIndex+baseAngle;var x=x0+rx*Math.cos(radian);var y=y0+ry*Math.sin(radian);return{x:x,
y:y}},getPointsOnBoundary:function(enhancedBoundary,vertexNumber){var points=[];for(var i=0;i<vertexNumber;i++){var cx=enhancedBoundary.cx;var cy=enhancedBoundary.cy;var rx=enhancedBoundary.rx;var ry=enhancedBoundary.ry;points.push(this.getPointOnCircle(cx,cy,rx,ry,vertexNumber,i))}var enhancedPointBoundary=QStage.enhanceBoundary(this.getPointBoundary(points));this.transformPointsToBoundary(points,this.transFrame.getPosition(),enhancedPointBoundary,enhancedBoundary);return points}};var Polygon=BaseShape.extend(polygon_prototype);
var polyline_prototype={getType:function(){return ShapeTypes.Polyline},init:function(parent,transformInfo,pointsInArray,initialProperties){var that=this;this._super(parent,transformInfo,pointsInArray,initialProperties);if(initialProperties==null)this.prop("applyFill",false);this.render()},toString:function(serializeType){var baseResult=this._super(serializeType);if(baseResult!==null)return baseResult;var position=this.transFrame.getPosition();var boundary=QStage.enhanceBoundary(this.transFrame.getBoundary(),
true);var rotation=this.transFrame.getRotation();if(serializeType===SerializeTypes.SVG)return"[SVG "+this.toString(SerializeTypes.TypeOnly)+"]";if(serializeType===SerializeTypes.Canvas)return"[Canvas "+this.toString(SerializeTypes.TypeOnly)+"]";return null},render:function(){var that=this;this.shapeElement=this.svgGroup.createChildNS("path",{"data-label":"Shape",d:this.getPathData(false),fill:this.prop("applyFill")?QStage.brushToRgba(this.prop("fill")):"none",stroke:QStage.brushToRgba(this.prop("stroke")),
...
d+=QStage.buildPathCommand("H",[boundary.right]);d+=QStage.buildPathCommand("V",[boundary.bottom]);d+=QStage.buildPathCommand("H",[boundary.left]);d+=QStage.buildPathCommand("Z");return d},toString:function(serializeType){var baseResult=this._super(serializeType);if(baseResult!==null)return baseResult;var position=this.transFrame.getPosition();var boundary=QStage.enhanceBoundary(this.transFrame.getBoundary(),true);var rotation=this.transFrame.getRotation();if(serializeType===SerializeTypes.SVG){var output=
"<rect";var pos=this.transFrame.getPosition();var absX=pos.x+boundary.left;var absY=pos.y+boundary.top;output+=' {0}="{1}"'.format("x",absX);output+=' {0}="{1}"'.format("y",absY);output+=' {0}="{1}"'.format("width",boundary.width);output+=' {0}="{1}"'.format("height",boundary.height);output+=' {0}="{1}"'.format("fill",QStage.brushToRgba(this.prop("fill")));output+=' {0}="{1}"'.format("stroke",QStage.brushToRgba(this.prop("stroke")));output+=' {0}="{1}"'.format("stroke-width",this.prop("stroke").width);
if(rotation&&rotation.angle%360!==0){var cx=pos.x+boundary.left+boundary.width/2;var cy=pos.y+boundary.top+boundary.height/2;var rotStr="rotate({0} {1} {2})".format(rotation.angle,cx,cy);output+=' {0}="{1}"'.format("transform",rotStr)}output+=' {0}="{1}"'.format("data-label","Shape");output+=" />";return output}if(serializeType===SerializeTypes.Canvas)return"[Canvas "+this.toString(SerializeTypes.TypeOnly)+"]";return null},render:function(){var that=this;this.shapeElement=this.svgGroup.createChildNS("path",
{"data-label":"Shape",fill:QStage.brushToRgba(this.prop("fill")),stroke:QStage.brushToRgba(this.prop("stroke")),"stroke-width":this.prop("stroke").width,d:this.getPath()});this.renderComplete()}};var Rectangle=BaseShape.extend(rectangle_prototype);function StartPage(workspace){this.workspace=workspace;this.wrapper=null;this.width=0;this.height=0}
StartPage.prototype={renderInto:function(wrapper,width,height){var that=this;this.wrapper=wrapper;this.resize(width,height);var css3support=QStage.supportsTransitions();if(css3support&&css3support.prefixedProperty){this.wrapper.style[css3support.prefixedProperty]="left 0.5s ease-in";this.wrapper.addEventListener(css3support.prefixedEventName||"transitionend",function(){})}var title=this.wrapper.createChild("div",{class:"title"});var appIcon=title.createChild("div",{id:"app-icon"});this.renderAppIcon(appIcon);
title.createChild("div",{id:"app-name"},"QLike Stage");var menuWrapper=this.wrapper.createChild("div",{class:"menu"});this.renderMenuItem(menuWrapper,0);if(!Config.General.offlineMode)this.renderMenuItem(menuWrapper,1);this.renderMenuItem(menuWrapper,2);this.renderMenuItem(menuWrapper,3);this.renderMenuItem(menuWrapper,4);this.wrapper.createChild("div",{class:"footer"},"By QLike.com");return this.wrapper},resize:function(width,height){this.width=width;this.height=height;this.wrapper.updateStyle("width",
width+"px");this.wrapper.updateStyle("height",height+"px")},setDisplay:function(display){this.wrapper.updateStyle("left",display?"0px":"-{0}px".format(this.width));this.workspace.stageWrapper.updateStyle("left",display?"{0}px".format(this.width):"0px");if(display){var selected=document.querySelectorAll(".item-selected");for(var i=0;i<selected.length;i++)selected[i].removeClassName("item-selected")}},openProject:function(id){var that=this;if(id&&id.length>0){this.workspace.reset(true);QStage.setUrlHash("id",
id);this.workspace.loadProject(id,function(){that.setDisplay(false)})}else{this.workspace.newProject();this.setDisplay(false)}},renderMenuItem:function(wrapper,subId){var that=this;var label="";var subLabel="";switch(subId){case 0:label="New";subLabel="New Project";break;case 1:label="Retrieve";subLabel="Retrieve from Server";break;case 2:label="Open";subLabel="Upload Project File";break;case 3:label="Recent";subLabel=Config.General.offlineMode===true?"Recent Projects":"Recent Projects on Server";
break;case 4:label="Deleted";subLabel="Deleted Projects";break}var item=wrapper.createChild("div",{class:"item","data-id":subId});var title=item.createChild("div",{class:"label no-select"},label);title.addEventListener("click",function(){var item=this.parentNode;var items=item.parentNode.childNodes;for(var i=0;i<items.length;i++){if(items[i].nodeType!=1)continue;if(items[i].hasClassName("item-selected"))items[i].removeClassName("item-selected")}item.addClassName("item-selected");var id=item.getAttribute("data-id");
if(id=="3")that.refreshProjectList();else if(id=="4")that.refreshTrashList()},false);var sub=item.createChild("div",{class:"sub"});sub.createChild("div",{class:"sub-label"},subLabel);var subContent=sub.createChild("div",{class:"sub-content"});switch(subId){case 0:var line=subContent.createChild("div",{class:"sub-item"});line.createChild("a",{class:"link",href:"javascript:void(0)"},"Create a new project from scratch").addEventListener("click",function(){that.openProject()},false);break;case 1:var line=
subContent.createChild("div",{class:"sub-item"});var form=line.createChild("form",{action:QStage.getAppUrl()});var txtId=form.createChild("input",{type:"text",class:"text",placeholder:"Project ID",name:"id"});var btnGo=form.createChild("input",{type:"button",class:"button",value:"Retrieve"});btnGo.addEventListener("click",function(){if(QStage.isValidId(txtId.value))that.openProject(txtId.value)},false);break;case 2:if(Config.General.offlineMode===true){var line=subContent.createChild("div",{class:"sub-item"});
var fileInput=line.createChild("input",{type:"file",class:"file",id:"fileToUpload",accept:".json"});var handleFileUpload=function(){var file=fileInput.files[0];if(!file)return;var reader=new FileReader;reader.onload=function(e){try{var fileContent=e.target.result;var jsonObj=null;var projectId=null;try{jsonObj=JSON.parse(fileContent);var filename=file.name;var match=filename.match(/project_([^.]+)\.json/);if(match&&QStage.isValidId(match[1]))projectId=match[1];else projectId=QStage.generateNewProjectId()}catch(jsonError){fileContent=
fileContent.replace(/^\u00ef\u00bb\u00bf/,"");if(fileContent.length>=32){projectId=fileContent.substring(0,32);var base64Payload=fileContent.substring(32);try{var decodedPayload=atob(base64Payload);jsonObj=JSON.parse(decodedPayload)}catch(decodeError){throw new Error("Failed to decode file. Invalid format.");}}else throw new Error("File format is invalid. File is too short.");}if(jsonObj&&projectId){that.workspace.reset();QStage.setUrlHash("id",projectId);QCache.setProject(projectId,jsonObj);that.workspace.painting.deserialize(jsonObj,
"JSON");that.setDisplay(false);fileInput.value=""}else throw new Error("Failed to parse project data.");}catch(error){alert("Error uploading file: "+error.message);fileInput.value=""}};reader.onerror=function(){alert("Error reading file.");fileInput.value=""};reader.readAsText(file)};fileInput.addEventListener("change",handleFileUpload,false)}else{var line=subContent.createChild("form",{class:"sub-item",action:"http://stage.qlike.com/repository/upload.php",method:"post",enctype:"multipart/form-data"});
line.createChild("input",{type:"file",class:"file",name:"fileToUpload",id:"fileToUpload",placeholder:"Project ID"});line.createChild("input",{type:"submit",class:"button",value:"Upload",name:"submit"})}break;case 3:subContent.createChild("div",{id:"project-list"});break;case 4:subContent.createChild("div",{id:"trash-list"});break}},refreshProjectList:function(){var that=this;var subContent=QStage.$("project-list");while(subContent&&subContent.childNodes.length>0)subContent.removeChild(subContent.firstChild);
if(!subContent)return;subContent.createChild("div",{class:"sub-item"},"Loading the list...");if(Config.General.offlineMode===true)try{var projectIds=QCache.getAllProjectIds();projectIds.sort(function(a,b){var projectA=QCache.getProject(a);var projectB=QCache.getProject(b);var timeA=projectA&&projectA.updatedAt?projectA.updatedAt:0;var timeB=projectB&&projectB.updatedAt?projectB.updatedAt:0;return timeB-timeA});while(subContent.childNodes.length>0)subContent.removeChild(subContent.firstChild);var activeProjects=
[];for(var i=0;i<projectIds.length;i++){var proj=QCache.getProject(projectIds[i]);if(!proj)continue;if(proj.active===false)continue;activeProjects.push({id:projectIds[i],project:proj})}if(activeProjects.length>0)for(var j=0;j<activeProjects.length;j++)(function(entry){var projectId=entry.id;var project=entry.project;var line=subContent.createChild("div",{class:"sub-item",style:"display: flex; align-items: center; justify-content: space-between;"});var itemContainer=line.createChild("div",{style:"display: flex; flex-direction: column;"});
var link=itemContainer.createChild("a",{class:"link",title:projectId,"data-id":projectId,href:"javascript:void(0)"},project.name?project.name:"[ID#"+projectId+"]");link.addEventListener("click",function(){that.openProject(this.getAttribute("data-id"))},false);itemContainer.createChild("div",{class:"last-updated"},"Created: "+formatRelativeTime(parseInt(projectId)));if(project.updatedAt)itemContainer.createChild("div",{class:"last-updated"},"Updated: "+formatRelativeTime(project.updatedAt));var recycleBtn=
line.createChild("button",{class:"button icon-button","data-id":projectId});var svg=document.createElementNS("http://www.w3.org/2000/svg","svg");svg.setAttribute("width","16");svg.setAttribute("height","16");svg.setAttribute("viewBox","0 0 16 16");svg.setAttribute("fill","currentColor");var path=document.createElementNS("http://www.w3.org/2000/svg","path");path.setAttribute("d","M6 2h4v1h3v1H2V3h3V2zM3 6h10l-1 8H4l-1-8zm2 1v6h1V7H5zm3 0v6h1V7H8z");svg.appendChild(path);recycleBtn.appendChild(svg);
recycleBtn.addEventListener("click",function(e){e.stopPropagation();var idToDeactivate=this.getAttribute("data-id");var project=QCache.getProject(idToDeactivate);if(project){project.active=false;if(QCache.setProject(idToDeactivate,project))that.refreshProjectList();else alert("Failed to delete project.")}},false)})(activeProjects[j]);else subContent.createChild("div",{class:"sub-item"},"No projects")}catch(err){log(err);while(subContent.childNodes.length>0)subContent.removeChild(subContent.firstChild);
subContent.createChild("div",{class:"sub-item"},"Error loading project list")}else{var url="http://stage.qlike.com/repository/?action=pull";QStage.sendPostRequest(url,null,function(responseText){try{while(subContent.childNodes.length>0)subContent.removeChild(subContent.firstChild);var obj=JSON.parse(responseText);for(var i=0;i<obj.payload.length;i++){var line=subContent.createChild("div",{class:"sub-item"});line.createChild("div",{class:"link",title:obj.payload[i].id,"data-id":obj.payload[i].id},
obj.payload[i].id).addEventListener("click",function(){that.openProject(this.getAttribute("data-id"))},false)}if(obj.payload.length<=0)subContent.createChild("div",{class:"sub-item"},"No record")}catch(e){log(e)}},function(errorCode){log(errorCode)})}},refreshTrashList:function(){var that=this;var subContent=QStage.$("trash-list");if(!subContent)return;while(subContent.childNodes.length>0)subContent.removeChild(subContent.firstChild);subContent.createChild("div",{class:"sub-item"},"Loading trash...");
if(Config.General.offlineMode===true)try{var allIds=QCache.getAllProjectIds();var inactiveProjects=[];for(var i=0;i<allIds.length;i++){var proj=QCache.getProject(allIds[i]);if(proj&&proj.active===false)inactiveProjects.push({id:allIds[i],project:proj})}while(subContent.childNodes.length>0)subContent.removeChild(subContent.firstChild);if(inactiveProjects.length===0){subContent.createChild("div",{class:"sub-item"},"No deleted projects");return}for(var j=0;j<inactiveProjects.length;j++)(function(entry){var projectId=
entry.id;var project=entry.project;var line=subContent.createChild("div",{class:"sub-item",style:"display: flex; align-items: center; justify-content: space-between;"});var itemContainer=line.createChild("div",{style:"display: flex; flex-direction: column;"});itemContainer.createChild("span",{},project.name?project.name:"[ID#"+projectId+"]");itemContainer.createChild("div",{class:"last-updated"},"ID: "+projectId);var restoreBtn=line.createChild("button",{class:"button icon-button","data-id":projectId});
var restoreSvg=document.createElementNS("http://www.w3.org/2000/svg","svg");restoreSvg.setAttribute("width","16");restoreSvg.setAttribute("height","16");restoreSvg.setAttribute("viewBox","0 0 16 16");restoreSvg.setAttribute("fill","currentColor");var restorePath=document.createElementNS("http://www.w3.org/2000/svg","path");restorePath.setAttribute("d","M8 3a5 5 0 1 0 4.546 2.914l1.42-1.42A7 7 0 1 1 8 1v2zm3.5-1h-4v4h4V2z");restoreSvg.appendChild(restorePath);restoreBtn.appendChild(restoreSvg);restoreBtn.addEventListener("click",
function(e){e.stopPropagation();var idToRestore=this.getAttribute("data-id");var projectToRestore=QCache.getProject(idToRestore);if(projectToRestore){projectToRestore.active=true;if(QCache.setProject(idToRestore,projectToRestore))that.refreshTrashList();else alert("Failed to restore project.")}},false)})(inactiveProjects[j]);var deleteAllBtnContainer=subContent.createChild("div",{style:"margin: 16px 8px;"});var deleteAllBtn=deleteAllBtnContainer.createChild("a",{class:"link",href:"javascript:void(0)"});
deleteAllBtn.textContent="Empty Trash";deleteAllBtn.addEventListener("click",function(){if(confirm("Are you sure you want to permanently delete all trashed projects? This action cannot be undone.")){var allIds2=QCache.getAllProjectIds();for(var k=0;k<allIds2.length;k++){var proj=QCache.getProject(allIds2[k]);if(proj&&proj.active===false)QCache.deleteProject(allIds2[k])}that.refreshTrashList()}},false)}catch(err){log(err);while(subContent.childNodes.length>0)subContent.removeChild(subContent.firstChild);
subContent.createChild("div",{class:"sub-item"},"Error loading trash")}else{while(subContent.childNodes.length>0)subContent.removeChild(subContent.firstChild);subContent.createChild("div",{class:"sub-item"},"Trash is managed on server. Use Retrieve to view server projects.")}},renderAppIcon:function(wrapper){var icon=wrapper.createChildNS("svg",{width:32,height:32});var g=icon.createChildNS("g");var size=32;var padding=size*10/300;var path="M{0},{1} H{2} L {3},{4} H {0} Z".format(size/3+padding,padding,
size*2/3,size-padding,size/3-padding);g.createChildNS("path",{fill:"#22CB20",stroke:"#22CB20","stroke-width":padding,"stroke-linejoin":"round",d:path});g.createChildNS("path",{fill:"#06ACD4",stroke:"#06ACD4","stroke-width":padding,"stroke-linejoin":"round",d:path,transform:"translate(0,0), rotate({1},{0},{0})".format(size/2,1*90)});g.createChildNS("path",{fill:"#FFCC00",stroke:"#FFCC00","stroke-width":padding,"stroke-linejoin":"round",d:path,transform:"translate(0,0), rotate({1},{0},{0})".format(size/
2,2*90)});g.createChildNS("path",{fill:"#FA5151",stroke:"#FA5151","stroke-width":padding,"stroke-linejoin":"round",d:path,transform:"translate(0,0), rotate({1},{0},{0})".format(size/2,3*90)});return icon}};
...
function Workspace(){this.painting=null;this.eventManager=null;this.drawingBoard=null;this.scaleplate=null;this.slidePane=null;this.objectManager=null;this.textPanel=null;this.promptDialog=null;this.startPage=null;this.width=0;this.height=0;this.stageWrapper=null;this.startWrapper=null;this.scrimWrapper=null;this.autoSaveEnabled=false;this.autoSaveTimer=null;this.init()}
Workspace.prototype={init:function(){this.painting=new Painting(this);this.eventManager=new EventManager(this);this.drawingBoard=new DrawingBoard(this);this.scaleplate=new Scaleplate(this,0,0);this.slidePane=new SlidePane(this);this.objectManager=new ObjectManager(this);this.textPanel=new TextPanel(this);this.promptDialog=new PromptDialog(this)},getExportRect:function(){return this.drawingBoard.getBoardSize()},newProject:function(){var defaultName="Untitled";var i=1;var newName=defaultName;while(QCache.isProjectNameExists(newName)){newName=
defaultName+" "+i;i++}this.painting.name=newName;this.slidePane.settingMenu.setProjectName(newName);var newId=QStage.generateNewProjectId();QStage.setUrlHash("id",newId);this.reset(true);this.autoSave()},reset:function(enableAutoSave){this.drawingBoard.reset();this.slidePane.reset();this.painting.reset();if(enableAutoSave)this.autoSaveEnabled=true},showStartPage:function(show){if(show)if(this.startPage==null){this.startPage=new StartPage(this);this.startPage.renderInto(this.startWrapper,this.width,
this.height);this.stageWrapper.updateStyle("left","{0}px".format(this.width))}else this.startPage.setDisplay(true);else if(this.startPage!=null)this.startPage.setDisplay(false)},render:function(){var that=this;this.width=document.documentElement.clientWidth-2*Config.Workspace.margin;this.height=document.documentElement.clientHeight-2*Config.Workspace.margin;var rootElement=document.body;this.stageWrapper=rootElement.createChild("div",{id:"workspace"});this.startWrapper=rootElement.createChild("div",
{id:"start-page"});this.scrimWrapper=rootElement.createChild("div",{class:"scrim",style:"display: none"});var css3support=QStage.supportsTransitions();this.stageWrapper.style[css3support.prefixedProperty]="left 0.5s ease-in";this.stageWrapper.addEventListener(css3support.prefixedEventName,function(){});this.slidePane.renderIntoWorkspace();this.scaleplate.renderIntoWorkspace();this.drawingBoard.renderIntoWorkspace();this.objectManager.renderIntoWorkspace();this.textPanel.renderIntoWorkspace();this.promptDialog.renderIntoWorkspace();
this.sizeChanged();this.painting.onShapeChanged=function(newShape){if(newShape!=null&&newShape.committed){that.slidePane.showPropertyMenu(newShape);that.painting.setFill(newShape.prop("fill").color,newShape.prop("fill").opacity);that.painting.setStroke(newShape.prop("stroke").color,newShape.prop("stroke").opacity,newShape.prop("stroke").width)}else that.slidePane.showPropertyMenu(null);if(newShape!=null)that.objectManager.selectShapes([newShape]);else that.objectManager.selectShapes([])};this.painting.onFillChanged=
function(color,opacity){that.slidePane.setFill(color,opacity)};this.painting.onStrokeChanged=function(color,opacity,width){that.slidePane.setStroke(color,opacity,width)};this.painting.onShapeAppended=function(appendedShape){that.objectManager.appendShape(appendedShape)};this.painting.onShapeRemoved=function(removedShape,removedIndex){that.objectManager.removeShape(removedShape,removedIndex)};this.painting.onShapeSorted=function(shape,oldIndex,newIndex){that.objectManager.sortShape(shape,oldIndex,
newIndex)};this.painting.onToolChanged=function(oldToolType,newToolType){that.slidePane.selectToolType(newToolType);if(newToolType==ToolTypes.Pen&&that.painting.selectedShape!=null&&that.painting.selectedShape.isPathShape()){that.painting.selectedShape.switchPathEditing(true);that.slidePane.showPropertyMenu(null)}else this.selectShape(null);if(oldToolType==ToolTypes.Crop)that.drawingBoard.canvasFrame.setEnabled(false);if(newToolType==ToolTypes.Crop)that.drawingBoard.canvasFrame.setEnabled(true)};
this.painting.onProjectChanged=function(){that.autoSave()};this.drawingBoard.onBoardMoved=function(offsetX,offsetY){that.scaleplate.changeOffset(offsetX,offsetY)};this.drawingBoard.onCaptured=function(){that.slidePane.selectMenuItem(null)};this.slidePane.fillMenu.onColorChanged=function(color,opacity,width){var shape=that.painting.selectedShape;if(shape!=null)shape.prop("fill",{color:color,opacity:opacity});that.painting.setFill(color,opacity)};this.slidePane.strokeMenu.onColorChanged=function(color,
opacity,width){var shape=that.painting.selectedShape;if(shape!=null)shape.prop("stroke",{color:color,opacity:opacity,width:width});that.painting.setStroke(color,opacity,width)};function checkWindowsize(){var nWidth=document.documentElement.clientWidth-2*Config.Workspace.margin;var nHeight=document.documentElement.clientHeight-2*Config.Workspace.margin;if(nWidth!=that.width||nHeight!=that.height){that.width=nWidth;that.height=nHeight;that.sizeChanged()}}setInterval(checkWindowsize,200);var animationNum=
QStage.getQueryStringByName("animation");if(animationNum!=null&&animationNum>0){var addIcon=function(){stage.addIcon();if(stage.icons.length<animationNum)setTimeout(addIcon,1E3)};if(animationNum>10)animationNum=10;var containerGroup=this.drawingBoard.underGroup;var svgWidth=this.drawingBoard.svgSize.width;var svgHeight=this.drawingBoard.svgSize.height;var stage=new Stage(containerGroup,svgWidth,svgHeight);addIcon();this.slidePane.wrapper.updateStyle("display","none")}this.loadWithUrlId()},saveToCache:function(){var projectId=
QStage.getUrlHash("id");if(!projectId||projectId.length===0)return false;var jsonObj=this.painting.serialize("JSON");var saved=QCache.setProject(projectId,jsonObj,true);return saved},getWrapper:function(){return this.stageWrapper},isSmallScreen:function(){return this.width<=480||this.height<=480},isLandscape:function(){return this.width>this.height},switchDeveloperMode:function(devMode){if(Config.General.developerMode==devMode)return;Config.General.developerMode=devMode;var displayStyle=devMode?null:
"none";QStage.$("hidden_map_tools").updateStyle("display",displayStyle);QStage.$("hidden_sample_menu").updateStyle("display",displayStyle)},sizeChanged:function(){this.stageWrapper.updateStyle("width",this.width);this.stageWrapper.updateStyle("height",this.height);this.stageWrapper.updateStyle("margin","{0}px 0px 0px {0}px".format(Config.Workspace.margin));if(this.startPage!=null)this.startPage.resize(this.width,this.height);this.textPanel.resize(this.width,this.height);this.promptDialog.resize(this.width,
this.height);var objMgrLeft=this.width-Config.ObjectManager.marginRight-Config.ObjectManager.width;this.objectManager.moveWindowTo(objMgrLeft,Config.ObjectManager.marginTop);this.resizeDrawingBoard(this.width,this.height);this.scaleplate.resize(this.width,this.height);this.slidePane.resize(this.width,this.height);if(this.scrimWrapper!=null){this.scrimWrapper.updateStyle("width",this.width+"px");this.scrimWrapper.updateStyle("height",this.height+"px");this.scrimWrapper.updateStyle("padding-top",this.height/
2+"px")}},resizeDrawingBoard:function(workspaceWidth,workspaceHeight){var scaleplateSize=Config.General.showScale?Config.Scaleplate.size:0;var boardWidth=workspaceWidth-scaleplateSize;var boardHeight=workspaceHeight-scaleplateSize;this.drawingBoard.resize(boardWidth,boardHeight)},transformPoint:function(point){return this.drawingBoard.transformPoint(point)},showGhost:function(d){return this.drawingBoard.showGhost(d)},getEventManager:function(owner){return this.eventManager.getRegistration(owner)},
...
if(!Config.General.offlineMode)return;var that=this;if(this.autoSaveTimer)clearTimeout(this.autoSaveTimer);this.autoSaveTimer=setTimeout(function(){try{that.saveToCache()}finally{that.autoSaveTimer=null}},Config.Workspace.autoSaveDelay)},loadProject:function(id,callback){var that=this;if(Config.General.offlineMode===true){var cachedObj=QCache.getProject(id);if(cachedObj!=null){log("Loaded from cache with ID:",id);that.painting.deserialize(cachedObj,"JSON");that.slidePane.settingMenu.setProjectName(that.painting.name);
if(typeof callback=="function")callback(true)}else{log("Project cache not found in cache with ID:",id);if(typeof callback=="function")callback(false)}}else{this.showScrim(true,"Loading... Please Wait...");this.loadFromServer(id,function(obj){if(obj!=null){log("Loaded from server with ID:",id);that.painting.deserialize(obj,"JSON");that.slidePane.settingMenu.setProjectName(that.painting.name);if(typeof callback=="function")callback(true)}else{log("Nothing loaded from server with ID:",id);if(typeof callback==
"function")callback(false)}that.showScrim(false)})}},loadWithUrlId:function(callback){var that=this;var urlId=QStage.getUrlHash("id");var shouldLoad=Config.General.offlineMode&&urlId&&urlId.length>0||!Config.General.offlineMode&&QStage.isValidId(urlId);if(shouldLoad)this.loadProject(urlId,function(success){if(!success)that.autoSaveEnabled=true;if(typeof callback==="function")callback(success)});else{this.autoSaveEnabled=true;if(typeof callback==="function")callback(false)}},loadFromServer:function(id,
callback){var that=this;var xmlhttp=new XMLHttpRequest;var url="http://stage.qlike.com/repository/?action=pull";var params="id="+id;xmlhttp.onreadystatechange=function(){if(xmlhttp.readyState==4)if(typeof callback=="function")if(xmlhttp.status==200)try{var o=JSON.parse(xmlhttp.responseText);if(o.payload.length>0){var projectJson=o.payload[0].json_string;callback(JSON.parse(projectJson))}else callback(null)}catch(err){log(xmlhttp.responseText);callback(null)}else callback(null)};xmlhttp.open("POST",
url,true);xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");xmlhttp.send(params)},sendToServer:function(callback){var that=this;this.showScrim(true,"Sending to server...");var xmlhttp=new XMLHttpRequest;var url="http://stage.qlike.com/repository/";var urlId=QStage.getUrlHash("id");if(QStage.isValidId(urlId))url+="?action=push&id="+urlId;else url+="?action=push";var params="payload="+this.painting.serialize("STRING");function processResponse(o){if(o!=null&&o.id&&o.rows>0){if(typeof callback==
"function")callback(o.id)}else callback(null);that.showScrim(false);that.painting.modified=false}xmlhttp.onreadystatechange=function(){if(xmlhttp.readyState==4)if(xmlhttp.status==200){var o=JSON.parse(xmlhttp.responseText);processResponse(o)}else processResponse(null)};xmlhttp.open("POST",url,true);xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");xmlhttp.send(params)},needToBeSaved:function(){var urlId=QStage.getUrlHash("id");return!QStage.isValidId(urlId)||this.painting.modified}};
var workspace=null;addEventListener("load",function(){workspace=new Workspace;workspace.render();var id=QStage.getUrlHash("id");if(id==null)workspace.showStartPage(true)});
